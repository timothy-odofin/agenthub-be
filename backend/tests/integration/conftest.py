...
class MongoDBTestMixin:
    ...
    async def cleanup_test_data(
        repository, user_prefix: str, database_name: str = None
    ):
        """Utility method for cleaning up test data."""
        try:
            if (
                not hasattr(repository, "_sessions_collection")
                or not repository._sessions_collection
            ):
                return
            ...
            def get_session_ids():
                cursor = repository._sessions_collection.find(
                    {"user_id": {"$regex": f"^{user_prefix}"}}, {"session_id": 1}
                )
...
def isolated_settings(
    ...
    test_config_files: Dict[str, Path],
...
async def mongodb_session_repository(test_database_name: str, test_user_prefix: str):
    ...
    await MongoDBTestMixin.cleanup_test_data(
        repository, test_user_prefix, test_database_name
    )
...
def mock_external_services():
    ...
    services = {"jira": Mock(), "confluence": Mock(), "github": Mock(), "slack": Mock()}
