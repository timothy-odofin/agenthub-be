...
from app.infrastructure.connections.database.mongodb_connection_manager import (
...
class TestMongoDBSessionIntegration:
    ...
    async def setup_and_teardown(self):
        ...
        self.test_db_uri = (
            f"{self.test_config.connection_string}/{self.test_database_name}"
        )
        ...
        self._original_db_name = (
            self.connection_manager.db_name
            if hasattr(self.connection_manager, "db_name")
            else None
        )
    ...
    async def _cleanup_test_resources(self):
        ...
        try:
            ...
            if (
                hasattr(self.repository, "_sessions_collection")
                and self.repository._sessions_collection
            ):
            ...
            print(
                f"Warning: Could not complete cleanup for test run {self.test_run_id}: {e}"
            )
            ...
            if hasattr(self.repository, "_database") and self.repository._database:
                ...
                def drop_collections():
                    ...
                    test_collections = [
                        ...
                        if "test" in col.lower() or self.test_run_id in col
                    ]
    ...
    async def _drop_test_database(self):
        ...
        try:
            if (
                hasattr(self.connection_manager, "_client")
                and self.connection_manager._client
            ):

                def drop_database():
                    self.connection_manager._client.drop_database(
    ...
    async def test_message_storage_and_history_retrieval(self):
        ...
        # Act: Add messages
        message1_id = await self.repository.add_message(
            session_id, "user", "Hello, how are you?"
        )
        message2_id = await self.repository.add_message(
            session_id, "assistant", "I'm doing well, thank you!"
        )
        message3_id = await self.repository.add_message(
            session_id, "user", "Can you help me with Python?"
        )
    ...
    async def test_session_update(self):
        ...
        success = await self.repository._update_session_async(
    ...
    async def test_session_deletion(self):
        ...
        test_session_still_exists = any(
            s.session_id == session_id for s in sessions_after
        )
    ...
    async def test_multiple_users_isolation(self):
        ...
        cross_access_history = await self.repository.get_session_history(
            user1_id, session2_id
        )
    ...
    async def test_complete_session_workflow(self):
        ...
        assert (
            history[3].content == "Python is known for its simplicity and readability."
        )
        ...
        delete_success = await self.repository._delete_session_async(
            user_id, session_id
        )
