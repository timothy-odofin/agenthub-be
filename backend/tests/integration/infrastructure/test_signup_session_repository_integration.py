...
class TestSignupSessionRepositoryIntegration:
    ...
    async def test_update_field_real_redis(self):
        ...
        await signup_session_repository.create_session(
            session_id, {"current_step": "EMAIL"}
        )

        # Update field
        await signup_session_repository.update_field(
            session_id, "email", "updated@test.com"
        )
    ...
    async def test_delete_session_real_redis(self):
        ...
        # Create session
        await signup_session_repository.create_session(
            session_id, {"email": "delete@test.com"}
        )
    ...
    async def test_extend_ttl_real_redis(self):
        ...
        # Create session
        await signup_session_repository.create_session(
            session_id, {"email": "extend@test.com"}
        )
        ...
        assert (
            595 <= ttl <= 605
        ), f"TTL should be ~600 seconds after extension, got {ttl}"
    ...
    async def test_session_expires_after_ttl(self):
        ...
        await signup_session_repository.create_session(
            session_id, {"email": "expire@test.com"}
        )
    ...
    async def test_get_nonexistent_session_returns_none(self):
        ...
        retrieved = await signup_session_repository.get_session(
    ...
    async def test_update_nonexistent_session_raises_error(self):
        ...
        with pytest.raises(ValueError, match="Session .* not found"):
            await signup_session_repository.update_field(
                "nonexistent_session_999", "email", "test@test.com"
            )
    ...
    async def test_session_exists_for_nonexistent_session(self):
        ...
        exists = await signup_session_repository.session_exists(
            "nonexistent_session_999"
        )
    ...
    async def test_timestamps_are_updated_correctly(self):
        ...
        await signup_session_repository.create_session(
            session_id, {"email": "timestamp@test.com"}
        )
        ...
        await signup_session_repository.update_field(
            session_id, "username", "timestampuser"
        )
    ...
    async def test_increment_attempt_counter(self):
        ...
        # Create session
        await signup_session_repository.create_session(
            session_id, {"email": "attempts@test.com"}
        )
        ...
        # Increment attempts
        await signup_session_repository.increment_attempt(
            session_id, "email_validation"
        )
    ...
    async def test_complete_signup_flow_simulation(self):
        ...
        # Step 2: Email
        await signup_session_repository.update_field(
            session_id, "email", "fullflow@test.com"
        )
        await signup_session_repository.update_field(
            session_id, "current_step", "USERNAME"
        )

        # Step 3: Username
        await signup_session_repository.update_field(
            session_id, "username", "fullflowuser"
        )
        await signup_session_repository.update_field(
            session_id, "current_step", "PASSWORD"
        )

        # Step 4: Password (hashed)
        await signup_session_repository.update_field(
            session_id, "password_hash", "hashed_password_123"
        )
        await signup_session_repository.update_field(
            session_id, "current_step", "FIRSTNAME"
        )

        # Step 5: First name
        await signup_session_repository.update_field(session_id, "firstname", "John")
        await signup_session_repository.update_field(
            session_id, "current_step", "LASTNAME"
        )

        # Step 6: Last name
        await signup_session_repository.update_field(session_id, "lastname", "Doe")
        await signup_session_repository.update_field(
            session_id, "current_step", "COMPLETE"
        )
    ...
    async def test_concurrent_field_updates(self):
        ...
        await asyncio.gather(
            signup_session_repository.update_field(
                session_id, "email", "concurrent@test.com"
            ),
            signup_session_repository.update_field(
                session_id, "username", "concurrentuser"
            ),
            signup_session_repository.update_field(
                session_id, "firstname", "Concurrent"
            ),
        )
    ...
    async def test_session_isolation(self):
        ...
        await signup_session_repository.create_session(
            session_id_1, {"email": "user1@test.com", "username": "user1"}
        )
        await signup_session_repository.create_session(
            session_id_2, {"email": "user2@test.com", "username": "user2"}
        )
