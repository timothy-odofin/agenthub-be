...
class TestJiraCommentIntegration:
    ...
    def setup_and_teardown(self):
        ...
        if self.added_comments:
            logger.info(
                f"Added {len(self.added_comments)} test comments during integration tests"
            )
    ...
    def test_add_simple_comment(self):
        ...
        try:
            ...
            result_json = self.jira_tools._add_comment(
                issue_key=self.TEST_ISSUE_KEY, comment_body=comment_text
            )
    ...
    def test_add_comment_with_on_behalf_of(self):
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        comment_text = (
            f"Integration test comment - With user context\nTimestamp: {timestamp}"
        )
        ...
        try:
            ...
            self.added_comments.append(
                f"'On behalf of' comment on {self.TEST_ISSUE_KEY}"
            )
    ...
    def test_search_users(self):
        ...
        try:
            ...
            result_json = self.jira_tools._search_users(
                query=self.TEST_USER_SEARCH, max_results=10
            )
            ...
            assert (
                len(result["users"]) > 0
            ), f"No users found matching '{self.TEST_USER_SEARCH}'"

            # Find the specific user
            users = result["users"]
            logger.info(
                f"✅ Found {len(users)} user(s) matching '{self.TEST_USER_SEARCH}':"
            )
    ...
    def test_get_all_users(self):
        ...
        try:
            result_json = self.jira_tools._get_all_users(start_at=0, max_results=10)
    ...
    def test_get_project_users(self):
        ...
        try:
            result_json = self.jira_tools._get_project_users(
                project_key=self.TEST_PROJECT_KEY, max_results=20
            )
            ...
            users = result["users"]
            logger.info(
                f"✅ Found {len(users)} user(s) with access to project {self.TEST_PROJECT_KEY}"
            )
            ...
            if user_found:
                logger.info(
                    f"✅ Test user '{self.TEST_USER_SEARCH}' has access to the project"
                )
            else:
                logger.warning(
                    f"⚠️  Test user '{self.TEST_USER_SEARCH}' not found in project users"
                )
    ...
    def test_add_comment_with_mention_adf(self):
        ...
        search_result_json = self.jira_tools._search_users(
            query=self.TEST_USER_SEARCH, max_results=5
        )
        ...
        logger.info(
            f"Found user for mention: {display_name} (Account ID: {account_id})"
        )
        ...
        try:
            ...
            result_json = self.jira_tools._add_comment(
                issue_key=self.TEST_ISSUE_KEY, comment_body=json.dumps(adf)
            )
            ...
            self.added_comments.append(
                f"Comment with @{display_name} mention on {self.TEST_ISSUE_KEY}"
            )
    ...
    def test_add_comment_with_multiple_mentions(self):
        ...
        users_result_json = self.jira_tools._get_project_users(
            project_key=self.TEST_PROJECT_KEY, max_results=5
        )
        ...
        try:
            ...
            logger.info(
                f"   Mentioned: {', '.join(f'@{name}' for name in mentioned_names)}"
            )
            logger.info(f"   Comment ID: {result['comment'].get('id')}")
    ...
    def test_comprehensive_comment_workflow(self):
        ...
        try:
            ...
            users_result_json = self.jira_tools._get_project_users(
                project_key=self.TEST_PROJECT_KEY, max_results=10
            )
            users_result = json.loads(users_result_json)
            assert len(users_result["users"]) > 0
            logger.info(
                f"Step 2 ✅ Retrieved {len(users_result['users'])} project users"
            )
            ...
            if not search_result["users"]:
                logger.warning(
                    f"Test user '{self.TEST_USER_SEARCH}' not found, using first project user"
                )
            ...
            logger.info(
                "🎉 Comprehensive comment workflow test completed successfully!"
            )
...
class TestJiraErrorHandling:
    ...
    def test_add_comment_invalid_issue(self):
        ...
        try:
            result_json = self.jira_tools._add_comment(
                issue_key="INVALID-99999", comment_body="Test comment"
            )
    ...
    def test_add_comment_empty_body(self):
        ...
        result_json = self.jira_tools._add_comment(
            issue_key="SCRUM-15", comment_body=""
        )
