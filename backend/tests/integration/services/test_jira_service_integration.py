...
class TestJiraServiceIntegration:
    ...
    def setup_and_teardown(self):
        ...
        if self.created_issues:
            logger.info(
                f"Created {len(self.created_issues)} test issues during integration tests"
            )
    ...
    def test_jira_server_connection(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Jira server connection successful. Version: {server_info.get('version')}"
            )
    ...
    def test_get_jira_projects(self):
        ...
        try:
            ...
            logger.info(
                f"Sample project: {first_project['key']} - {first_project['name']}"
            )
    ...
    def test_create_jira_issue(self):
        ...
        # Create test issue
        test_summary = (
            f"Integration Test Issue - {datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        test_description = (
            "This is a test issue created by JiraService integration tests"
        )
        ...
        try:
            ...
            if any(
                ...
                for keyword in ["summary", "fields", "screen", "configuration"]
            ):
                logger.warning(
                    f"Skipping issue creation test - project configuration issue: {e}"
                )
                pytest.skip(
                    f"Project configuration doesn't support issue creation: {e}"
                )
            else:
    ...
    def test_search_issues_with_jql(self):
        ...
        try:
            search_results = jira.search_issues(
                jql=jql_query, limit=10, fields=["key", "summary", "status", "created"]
            )
            ...
            for field in required_fields:
                assert (
                    field in search_results
                ), f"Search results missing '{field}' field"
            ...
            if issues:
                ...
                logger.info(
                    f"Sample issue: {first_issue['key']} - {first_issue['fields'].get('summary', 'No summary')}"
                )
    ...
    def test_search_issues_with_complex_jql(self):
        ...
        for jql in jql_queries:
            try:
                ...
                logger.info(
                    f"✅ Complex JQL query successful: {jql[:50]}... Found {len(results['issues'])} issues"
                )

            except Exception as e:
                logger.warning(
                    f"JQL query failed (this might be expected): {jql} - Error: {e}"
                )
    ...
    def test_jira_error_handling(self):
        ...
        try:
            ...
            logger.info(
                f"✅ Correctly handled non-existent issue error: {type(e).__name__}"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Correctly handled non-existent project error: {type(e).__name__}"
            )
    ...
    def test_comprehensive_workflow(self):
        ...
        try:
            ...
            logger.info(
                f"Step 2 ✅ Using project: {test_project['key']} - {test_project['name']}"
            )
            ...
            logger.info(
                f"Step 3 ✅ Found {len(existing_issues['issues'])} existing issues"
            )

            # Step 4: Create a new issue
            workflow_summary = (
                f"Workflow Test Issue - {datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            logger.info(
                f"Step 5 ✅ Retrieved and validated issue: {workflow_issue_key}"
            )
        ...
        try:
            ...
            for limit in limits:
                ...
                results = jira.search_issues(
                    jql=f"project = {project_key} ORDER BY created DESC", limit=limit
                )
                ...
                logger.info(
                    f"✅ Limit {limit}: Retrieved {len(results['issues'])} issues in {duration:.2f}s"
                )
...
class TestJiraToolsIntegration:
    ...
    def setup_and_teardown(self):
        ...
        if self.created_issues:
            logger.info(
                f"Created {len(self.created_issues)} test issues during JiraTools integration tests"
            )
    ...
    def test_jira_tools_initialization(self):
        ...
        try:
            ...
            logger.info(
                f"✅ JiraTools initialized with {len(tools)} tools: {actual_tool_names}"
            )
    ...
    def test_get_jira_projects_tool(self):
        ...
        try:
            ...
            logger.info(
                f"✅ get_jira_projects tool successful. Found {response_data['total_projects']} projects"
            )
            logger.info(
                f"Sample project: {response_data['projects'][0]['key']} - {response_data['projects'][0]['name']}"
            )
    ...
    def test_create_jira_issue_tool(self):
        ...
        try:
            ...
            test_summary = (
                f"Agent Tool Test Issue - {datetime.now().strftime('%Y%m%d_%H%M%S')}"
            )
            ...
            logger.info(
                f"✅ create_jira_issue tool successful. Created issue: {issue_key}"
            )
            ...
            if "summary" in str(e) or "project" in str(e).lower():
                logger.warning(
                    f"create_jira_issue tool failed due to project configuration: {e}"
                )
    ...
    def test_get_jira_issue_tool(self):
        ...
        try:
            ...
            logger.info(
                f"✅ get_jira_issue tool successful for issue: {test_issue_key}"
            )
    ...
    def test_search_jira_issues_tool(self):
        ...
        try:
            ...
            result = self.jira_tools._search_issues(jql=jql_query, max_results=5)
            ...
            logger.info(
                f"✅ search_jira_issues tool successful. Found {len(issues)} issues with JQL: {jql_query}"
            )
    ...
    def test_jira_tools_error_handling(self):
        ...
        try:
            result = self.jira_tools._search_issues(
                jql="nonexistentfield = 'test'", max_results=10
            )
    ...
    def test_jira_tools_comprehensive_workflow(self):
        ...
        try:
            ...
            logger.info(
                f"Step 1 ✅ Using project: {test_project['key']} - {test_project['name']}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Step 3 ⚠️ Issue creation failed (expected): {create_error}"
                )
