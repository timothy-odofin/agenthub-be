...
class TestAPIIntegration(MongoDBTestMixin):
    ...
    async def test_get_session_history_endpoint(self):
        ...
        async with create_test_client() as client:
            response = await client.get(
                f"/api/v1/chat/sessions/{test_session_id}/history"
            )
    ...
    async def test_get_user_sessions_endpoint(self):
        ...
        async with create_test_client() as client:
            response = await client.get(
                f"/api/v1/chat/users/{self.test_user_id_1}/sessions"
            )
            ...
            for result in results:
                if isinstance(result, Exception):
                    ...
                    assert status_code in [
                        ...
                        500,
                        503,
                    ], f"Unexpected status for {user_id}: {status_code}"
    ...
    async def test_api_request_size_limits(self):
        ...
        test_data = {"message": large_message, "user_id": self.test_user_id_1}
        ...
        async with create_test_client() as client:
            ...
            assert response.status_code in [
                ...
                413,
                500,
            ]  # Success, too large, or service error
    ...
    async def test_async_concurrent_sessions(self):
        ...
        async def create_and_use_session(client: AsyncClient, user_id: str):
            ...
            session_response = await client.post(
                "/api/v1/chat/sessions", json=session_data
            )

            # Then send message
            message_data = {"message": f"Hello from {user_id}", "user_id": user_id}

            message_response = await client.post(
                "/api/v1/chat/message", json=message_data
            )
