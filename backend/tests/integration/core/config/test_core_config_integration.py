...
from app.core.config import DynamicConfig, Settings, YamlLoader
from app.core.utils import EnvironmentManager, PropertyResolver, env
...
class TestSettingsFrameworkIntegration(TestCoreConfigurationSystemIntegration):
    ...
    def test_settings_singleton_behavior(self):
        ...
        with patch.object(
            Settings, "_get_resources_directory", return_value=self.resources_dir
        ):
            Settings.reset()
    ...
    def test_nested_profile_discovery(self):
        ...
        with patch.object(
            ...
            assert settings.workflows.approval.steps == [
                "validate",
                "review",
                "approve",
            ]
    ...
    def test_settings_caching_behavior(self):
        ...
        self._create_profile_file(
            "cache-test",
            {"database": {"pool_size": 10}, "redis": {"max_connections": 100}},
        )
        ...
        with patch.object(
            ...
            assert (
                second_access_time < first_access_time * 0.5
...
class TestPropertyResolutionIntegration(TestCoreConfigurationSystemIntegration):
    ...
    def test_complex_placeholder_resolution(self):
        ...
        with patch.object(
            ...
            assert (
                settings.complex_placeholders.api.endpoint
            ...
            assert (
                settings.complex_placeholders.computed.display_name
                == "API v2 (3 retries)"
            )
    ...
    def test_type_conversion_integration(self):
        ...
        self._create_profile_file(
            "type-conversion",
            {
                ...
                "cache": {"ttl": "${CACHE_TTL}", "enabled": "${CACHE_ENABLED:true}"},
                "debug": "${DEBUG_ENABLED}",
                "features": "${FEATURES}",
                "static_values": {
                    "integer": 42,
...
class TestDynamicConfigurationIntegration(TestCoreConfigurationSystemIntegration):
    ...
    def test_dot_notation_access_patterns(self):
        ...
        config_data = {
            "api": {
                "v1": {
                    "endpoints": {"users": "/api/v1/users", "orders": "/api/v1/orders"},
                    "rate_limits": {"per_minute": 60, "per_hour": 3600},
                }
            },
            "features": {"authentication": True, "analytics": False},
        }
...
class TestConfigurationIntegrationWithApplicationComponents(
    TestCoreConfigurationSystemIntegration
):
    ...
    def test_integration_with_existing_config_providers(self):
        ...
        self._create_profile_file(
            "app",
            {
                ...
                "version": "1.0.0",
                "debug": False,
                "security": {"jwt_secret": "test_secret", "token_expiry": 3600},
                "api": {"prefix": "/api/v1", "cors_enabled": True},
            },
        )
        ...
        with patch.object(
            ...
            from app.core.config import app_config, config
    ...
    def test_configuration_reload_and_hot_updates(self):
        ...
        self._create_profile_file(
            "hot-reload",
            {
                "feature_flags": {"new_ui": False, "advanced_analytics": True},
...
class TestConfigurationErrorHandlingAndEdgeCases(
    TestCoreConfigurationSystemIntegration
):
    ...
    def test_circular_reference_detection(self):
        ...
        self._create_env_vars(
            {"CIRCULAR_A": "${CIRCULAR_B}", "CIRCULAR_B": "${CIRCULAR_A}"}
        )

        self._create_profile_file(
            "circular",
            {"config": {"value_a": "${CIRCULAR_A}", "value_b": "${CIRCULAR_B}"}},
        )
...
class TestConfigurationPerformanceIntegration(TestCoreConfigurationSystemIntegration):
    ...
    def test_concurrent_access_behavior(self):
        ...
        self._create_profile_file(
            "concurrent", {"shared": {"counter": 0, "data": "shared_value"}}
        )
