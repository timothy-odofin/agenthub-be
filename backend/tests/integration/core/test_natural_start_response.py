...
from app.schemas.conversational_auth import ConversationalSignupRequest, SignupStep
...
class TestNaturalStartResponse:
    ...
    async def test_what_do_i_need_question(self, conversational_auth_service):
        ...
        assert (
            "assistant" not in message_lower or "signup assistant" not in message_lower
        )

        # Should contain helpful information about requirements
        assert any(
            word in message_lower for word in ["email", "username", "password", "name"]
        )
    ...
    async def test_what_information_needed(self, conversational_auth_service):
        ...
        if (
            ...
            or "first" in message_lower
            or "last" in message_lower
        ):
            requirements_mentioned += 1
    ...
    async def test_comparison_robotic_vs_natural(self, conversational_auth_service):
        ...
        for question in test_questions:
            request = ConversationalSignupRequest(
                message=question, session_id=None, current_step=SignupStep.START
            )
            ...
            has_robotic_language = any(
                phrase in message_lower for phrase in robotic_phrases
            )
            ...
            assert (
                not has_robotic_language
            ), f"Response contains robotic language for question: '{question}'"
