...
class TestIntelligentClarification:
    ...
    async def test_user_asks_why_email_needed(self, conversational_auth_service):
        ...
        start_request = ConversationalSignupRequest(
            message="yes", session_id=None, current_step=SignupStep.START
        )
        start_response = await conversational_auth_service.process_signup_step(
            start_request
        )
        ...
        assert any(
            ...
            for word in ["login", "account", "identifier", "contact"]
        )
    ...
    async def test_user_asks_about_password_requirements(
    ...
    async def test_user_asks_vs_provides_data(self, conversational_auth_service):
        ...
        question_response = await conversational_auth_service.process_signup_step(
            question_request
        )
        ...
        data_response = await conversational_auth_service.process_signup_step(
            data_request
        )
