...
class TestFileIngestionServiceArchitecture:
    ...
    def test_initialization_sets_required_attributes(self, mock_settings):
        ...
        with (
            ...
            patch(
                "app.services.ingestion.file_data_source_util._construct_mapping"
            ) as mock_mapping,
            patch(
                "app.db.vector.providers.db_provider.VectorStoreFactory.get_default_vector_store"
...
class TestFileIngestionServiceValidation:
    ...
    def test_validate_config_no_sources(self, mock_settings):
        ...
        with pytest.raises(
            ValueError, match="No sources paths provided in configuration"
        ):
    ...
    def test_validate_config_nonexistent_path(self, mock_settings):
        ...
        with patch("os.path.exists", return_value=False):
            with pytest.raises(
                ValueError, match="Source path does not exist: /nonexistent/path"
            ):
                FileIngestionService()
...
class TestFileIngestionServiceDocumentLoading:
    ...
    def test_load_document_text_file(self, mock_settings):
        ...
        mock_document = Document(
            page_content="Test content", metadata={"source": "/test/file.txt"}
        )
    ...
    def test_load_document_pdf_with_config(self, mock_settings):
        ...
        mock_loader = Mock()
        mock_document = Document(
            page_content="PDF content", metadata={"source": "/test/file.pdf"}
        )
        mock_loader.load.return_value = [mock_document]
    ...
    def test_handle_zip_archive(self, mock_settings):
        ...
        with (
            ...
            mock_document = Document(
                ...
                metadata={"source": "/temp/extracted.txt"},
            )
            mock_loader.load.return_value = [mock_document]
            ...
            with (
                ...
                assert (
                    documents[0].metadata.get("archive_source") == "/test/archive.zip"
                )
...
class TestFileIngestionServiceSingleFileProcessing:
    ...
    async def test_ingest_single_success(self, mock_settings):
        ...
        with patch("os.path.exists", return_value=True):
            ...
            mock_vector_store.save_and_embed = AsyncMock(
                return_value=["doc_id_1", "doc_id_2"]
            )
            ...
            mock_documents = [
                Document(
                    page_content="Content 1", metadata={"source": "/test/file.txt"}
                ),
                Document(
                    page_content="Content 2", metadata={"source": "/test/file.txt"}
                ),
            ]
...
class TestFileIngestionServiceBatchProcessing:
    ...
    async def test_ingest_multiple_sources_success(self, mock_settings):
        ...
        with patch("os.path.exists", return_value=True):
            ...
            with patch.object(
                service, "_process_files_in_folder", return_value=True
            ) as mock_process:
                ...
                mock_process.assert_has_calls(
                    [call("/test/folder1"), call("/test/folder2")]
                )
    ...
    async def test_process_files_in_folder(self, mock_settings):
        ...
        with patch("os.path.exists", return_value=True):
            ...
            with patch.object(
                service, "ingest_single", return_value=True
            ) as mock_ingest_single:
                # Mock pathlib.Path used in the actual service module
                with patch(
                    "app.services.ingestion.file_ingestion_service.Path"
                ) as mock_path_class:
                    mock_path = Mock()
                    ...
                    mock_ingest_single.assert_has_calls(
                        [call("/test/file1.txt"), call("/test/file2.txt")]
                    )
    ...
    async def test_process_file_with_chunking(self, mock_settings):
        ...
        with patch("os.path.exists", return_value=True):
            ...
            original_doc = Document(
                page_content="Long content", metadata={"source": "/test/file.txt"}
            )
            chunk1 = Document(
                page_content="Long", metadata={"source": "/test/file.txt"}
            )
            chunk2 = Document(
                page_content="content", metadata={"source": "/test/file.txt"}
            )

            with (
                patch.object(
                    service, "_load_document", return_value=[original_doc]
                ) as mock_load,
                patch.object(
