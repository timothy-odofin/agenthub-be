...
class TestLLMServiceValidation:
    ...
    def test_validate_provider_configured_raises_for_ollama_missing_base_url(self):
        ...
        with pytest.raises(
            ValueError, match="not configured.*missing or invalid base_url"
        ):
...
class TestModelValidation:
    ...
    @patch("app.core.config.settings")
    def test_validate_model_raises_when_no_default_and_no_model_provided(
        self, mock_settings
    ):
    ...
    def test_validate_model_allows_any_model_when_no_versions_list(self, mock_settings):
        ...
        mock_openai = SimpleNamespace(
            api_key="sk-valid-key", model="gpt-4", model_versions=[]  # No restrictions
        )
        mock_settings.llm = SimpleNamespace(openai=mock_openai)
...
class TestGetAvailableProviders:
    ...
    def test_get_available_providers_with_ollama(self, mock_settings):
        ...
        mock_llm = SimpleNamespace(default=mock_default, ollama=mock_ollama)
        mock_settings.llm = mock_llm
    ...
    def test_get_available_providers_handles_missing_display_name(self, mock_settings):
        ...
        mock_llm = SimpleNamespace(default=mock_default, openai=mock_openai)
        mock_settings.llm = mock_llm
    ...
    @patch("app.core.config.settings")
    def test_get_available_providers_returns_empty_when_none_configured(
        self, mock_settings
    ):
