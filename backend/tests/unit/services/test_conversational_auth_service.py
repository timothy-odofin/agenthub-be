...
def mock_settings():
    ...
    mock_prompts.start = (
        "👋 Welcome! Let's create your account. What's your email address?"
    )
    mock_prompts.email_success = (
        "Great! Now choose a username (3-30 characters, letters, numbers, _ or -)."
    )
    ...
    mock_validation_errors.password_weak = (
        "❌ Password must be at least 8 characters and include: {requirements}."
    )
...
@pytest.fixture
def conversational_auth_service(
    mock_settings, mock_llm, mock_user_repository, mock_auth_service
):
    """Create ConversationalAuthService instance with mocked dependencies."""
    with (
        patch("src.app.services.conversational_auth_service.settings", mock_settings),
        patch(
            "src.app.services.conversational_auth_service.LLMFactory"
        ) as mock_factory,
        patch(
            "src.app.services.conversational_auth_service.user_repository",
            mock_user_repository,
        ),
        patch(
            "src.app.services.conversational_auth_service.auth_service",
            mock_auth_service,
        ),
    ):
...
class TestConversationalAuthServiceInit:
    ...
    def test_init_assigns_correct_handlers(self, conversational_auth_service):
        ...
        assert (
            conversational_auth_service._step_handlers[SignupStep.EMAIL]
            == conversational_auth_service._process_email
        )
        assert (
            conversational_auth_service._step_handlers[SignupStep.USERNAME]
            == conversational_auth_service._process_username
        )
        assert (
            conversational_auth_service._step_handlers[SignupStep.PASSWORD]
            == conversational_auth_service._process_password
        )
        assert (
            conversational_auth_service._step_handlers[SignupStep.FIRSTNAME]
            == conversational_auth_service._process_firstname
        )
        assert (
            conversational_auth_service._step_handlers[SignupStep.LASTNAME]
            == conversational_auth_service._process_lastname
        )
...
class TestLLMExtraction:
    ...
    @pytest.mark.asyncio
    async def test_extract_field_from_message_success(
        self, conversational_auth_service
    ):
    ...
    async def test_extract_field_removes_quotes(self, conversational_auth_service):
        ...
        # Act
        result = await conversational_auth_service._extract_field_from_message(
            "You can call me johndoe", "username"
        )
    ...
    @pytest.mark.asyncio
    async def test_extract_field_handles_single_quotes(
        self, conversational_auth_service
    ):
...
class TestProcessSignupStepRegistryPattern:
    ...
    async def test_process_start_step(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="Hi", current_step=SignupStep.START
        )
    ...
    async def test_process_step_invalid_step(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="test", current_step=SignupStep.EMAIL
        )
...
class TestEmailValidation:
    ...
    async def test_process_email_valid_and_unique(self, conversational_auth_service):
        ...
        with patch(
            ...
            request = ConversationalSignupRequest(
                message="My email is john@example.com", current_step=SignupStep.EMAIL
            )

            # Act
            response = await conversational_auth_service._process_email(
                request, "session-123"
            )
    ...
    async def test_process_email_invalid_format(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="not-an-email", current_step=SignupStep.EMAIL
        )
    ...
    async def test_process_email_already_exists(self, conversational_auth_service):
        ...
        with patch(
            ...
            request = ConversationalSignupRequest(
                message="john@example.com", current_step=SignupStep.EMAIL
            )
...
class TestUsernameValidation:
    ...
    async def test_process_username_valid_and_unique(self, conversational_auth_service):
        ...
        with patch(
            ...
            request = ConversationalSignupRequest(
                message="Call me johndoe", current_step=SignupStep.USERNAME
            )

            # Act
            response = await conversational_auth_service._process_username(
                request, "session-123"
            )
    ...
    async def test_process_username_invalid_format(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="ab", current_step=SignupStep.USERNAME
        )
    ...
    async def test_process_username_already_exists(self, conversational_auth_service):
        ...
        with patch(
            ...
            request = ConversationalSignupRequest(
                message="johndoe", current_step=SignupStep.USERNAME
            )
...
class TestPasswordValidationSimplified:
    ...
    async def test_process_password_valid(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="Password123", current_step=SignupStep.PASSWORD
        )

        # Act
        response = await conversational_auth_service._process_password(
            request, "session-123"
        )
    ...
    async def test_process_password_too_short(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="Pass1", current_step=SignupStep.PASSWORD
        )
    ...
    async def test_process_password_no_number(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="Password", current_step=SignupStep.PASSWORD
        )
    ...
    @pytest.mark.asyncio
    async def test_process_password_no_special_char_required(
        self, conversational_auth_service
    ):
...
class TestNameValidation:
    ...
    async def test_process_firstname_valid(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="John", current_step=SignupStep.FIRSTNAME
        )

        # Act
        response = await conversational_auth_service._process_firstname(
            request, "session-123"
        )
    ...
    async def test_process_firstname_invalid(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="123", current_step=SignupStep.FIRSTNAME
        )
    ...
    @pytest.mark.asyncio
    async def test_process_lastname_valid_completes_signup(
        self, conversational_auth_service
    ):
        ...
        with patch(
            ...
            # Act
            response = await conversational_auth_service._process_lastname(
                request, "session-123"
            )
    ...
    async def test_process_lastname_invalid(self, conversational_auth_service):
        ...
        request = ConversationalSignupRequest(
            message="", current_step=SignupStep.LASTNAME, firstname="John"
        )
...
class TestValidationPatterns:
    ...
    def test_email_pattern_valid(self, conversational_auth_service):
        ...
        for email in valid_emails:
            assert conversational_auth_service.EMAIL_PATTERN.match(
                email
    ...
    def test_email_pattern_invalid(self, conversational_auth_service):
        ...
        for email in invalid_emails:
            assert not conversational_auth_service.EMAIL_PATTERN.match(
                email
    ...
    def test_username_pattern_valid(self, conversational_auth_service):
        ...
        for username in valid_usernames:
            assert conversational_auth_service.USERNAME_PATTERN.match(
                username
            ), f"{username} should be valid"
    ...
    def test_username_pattern_invalid(self, conversational_auth_service):
        ...
        for username in invalid_usernames:
            assert not conversational_auth_service.USERNAME_PATTERN.match(
                username
    ...
    def test_password_pattern_simplified(self, conversational_auth_service):
        ...
        for password in valid_passwords:
            assert conversational_auth_service.PASSWORD_PATTERN.match(
                password
        ...
        for password in invalid_passwords:
            assert not conversational_auth_service.PASSWORD_PATTERN.match(
                password
            ), f"{password} should be invalid"
