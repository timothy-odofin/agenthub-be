...
class MockAgent:
    ...
    async def execute(
        self, user_input: str, session_id: str = None, protocol: str = None
    ):
...
class TestChatServiceBasicFunctionality:
    ...
    def test_create_session_success(self, clean_chat_service):
        ...
        with patch(
            "app.services.chat_service.SessionRepositoryFactory"
        ) as mock_factory:
            mock_repo = Mock()
    ...
    async def test_agent_lazy_initialization(self, clean_chat_service):
        ...
        with (
            patch("app.services.chat_service.AgentFactory") as mock_agent_factory,
