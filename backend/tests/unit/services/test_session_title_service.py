...
class TestLLMTitleGenerationStrategy:
    ...
    async def test_generate_title_success(self):
        ...
        with patch.object(
            strategy, "_build_title_generation_prompt", return_value="mock prompt"
        ):
            with patch(
                "app.llm.factory.llm_factory.LLMFactory.get_llm"
            ) as mock_get_llm:
                mock_llm = AsyncMock()
    ...
    async def test_generate_title_with_quotes(self):
        ...
        messages = [create_test_message(role="user", content="Help with Jira")]
    ...
    async def test_generate_title_too_many_words(self):
        ...
        messages = [create_test_message(role="user", content="Tell me about something")]
    ...
    async def test_generate_title_llm_failure_fallback(self):
        ...
        messages = [create_test_message(role="user", content="How to fix my computer?")]
    ...
    def test_clean_title_removes_trailing_punctuation(self):
        ...
        assert (
            strategy._clean_title("Password Reset Question?")
...
class TestExtractiveTitleGenerationStrategy:
    ...
    async def test_generate_title_truncates_long_message(self):
        ...
        messages = [create_test_message(role="user", content=long_message)]
...
class TestTimestampTitleGenerationStrategy:
    ...
    async def test_generate_timestamp_title(self):
        ...
        messages = [create_test_message(role="user", content="Hello")]
...
class TestSessionTitleService:
    ...
    async def test_generate_title_with_fallback_strategy(self):
        ...
        messages = [create_test_message(role="user", content="Help")]
        ...
        title = await service.generate_title_from_messages(
            messages, fallback_strategy=mock_fallback
        )
    ...
    async def test_auto_generate_and_update_title(self):
        ...
        title = await service.auto_generate_and_update_title(
            session_id=session_id, user_id=user_id, session_repository=mock_repo
        )
    ...
    async def test_auto_generate_handles_no_messages(self):
        ...
        title = await service.auto_generate_and_update_title(
            session_id="test-123", user_id="user-456", session_repository=mock_repo
        )
...
class TestTitleGenerationIntegration:
    ...
    async def test_strategy_pattern_flexibility(self):
        ...
        messages = [create_test_message(role="user", content="Test message")]
