...
def auth_service():
    ...
    with (
        patch("src.app.services.auth_service.user_repository") as mock_user_repo,
...
class TestAuthServiceSignup:
    ...
    async def test_signup_success(self, auth_service):
        ...
        auth_service.signup_workflow.ainvoke = AsyncMock(
            return_value={
                "success": True,
        ...
        auth_service.token_manager.create_refresh_token.return_value = (
            "refresh_token_456"
        )
        ...
        access_call_kwargs = (
            auth_service.token_manager.create_access_token.call_args.kwargs
        )
        ...
        refresh_call_kwargs = (
            auth_service.token_manager.create_refresh_token.call_args.kwargs
        )
...
class TestAuthServiceLogin:
    ...
    async def test_login_with_email_success(self, auth_service, mock_user):
        ...
        auth_service.user_repository.get_user_by_email = AsyncMock(
        ...
        # Verify method calls
        auth_service.user_repository.get_user_by_email.assert_called_once_with(
            identifier
        )
        auth_service.password_manager.verify_password.assert_called_once_with(
    ...
    async def test_login_with_username_success(self, auth_service, mock_user):
        ...
        auth_service.user_repository.get_user_by_username = AsyncMock(
            return_value=mock_user
        )
        auth_service.password_manager.verify_password.return_value = True
        auth_service.token_manager.create_access_token.return_value = "access_token"
        ...
        auth_service.user_repository.get_user_by_username.assert_called_once_with(
            identifier
        )
...
class TestAuthServiceRefreshToken:
    ...
    async def test_refresh_token_success(self, auth_service, mock_user):
        ...
        # Verify method calls
        auth_service.token_manager.verify_refresh_token.assert_called_once_with(
            refresh_token
        )
        auth_service.user_repository.get_user_by_id.assert_called_once_with(
            mock_user.id
        )
...
class TestAuthServiceIntegration:
    ...
    async def test_signup_then_login_workflow(self, auth_service, mock_user):
        ...
        auth_service.token_manager.create_access_token.return_value = (
            "signup_access_token"
        )
        auth_service.token_manager.create_refresh_token.return_value = (
