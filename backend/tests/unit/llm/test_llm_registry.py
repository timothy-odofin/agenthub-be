...
class TestLLMRegistry:
    ...
    def test_register_multiple_providers(self, clean_llm_registry):
        ...
        assert (
            LLMRegistry.get_provider_class(LLMProvider.ANTHROPIC)
    ...
    def test_get_provider_class_not_registered_raises_value_error(
        self, clean_llm_registry
    ):
        ...
        assert (
            "LLM provider 'LLMProvider.OPENAI' not found in registry" in error_message
        )
...
class TestLLMRegistryEdgeCases:
    ...
    def test_registry_thread_safety_simulation(self, clean_llm_registry):
        ...
        for _ in range(10):
            ...
            results.append(
                LLMRegistry.get_provider_class(LLMProvider.OPENAI) == ThreadSafeProvider
            )
    ...
    def test_ensure_providers_imported_handles_import_error(self, clean_llm_registry):
        ...
        with patch("builtins.__import__", side_effect=mock_import):
            try:
                ...
                pytest.fail(
                    "_ensure_providers_imported should handle import errors gracefully"
                )
