...
class TestContextWindow:
    ...
    def test_context_window_calculation(self):
        ...
        window = ContextWindow(
            max_tokens=1000, reserved_tokens=100, system_prompt_tokens=50
        )
    ...
    def test_context_window_edge_cases(self):
        ...
        # Very small window
        small_window = ContextWindow(
            max_tokens=100, reserved_tokens=50, system_prompt_tokens=40
        )
        assert small_window.available_tokens == 10

        # Zero available tokens
        zero_window = ContextWindow(
            max_tokens=100, reserved_tokens=60, system_prompt_tokens=50
        )
...
class TestSimpleTokenCounter:
    ...
    def test_token_estimation_accuracy(self):
        ...
        # Medium text
        medium_tokens = counter.count_tokens(
            "This is a medium length message with several words"
        )
...
class TestContextWindowConcepts:
    ...
    def test_token_counting_integration(self):
        ...
        counter = SimpleTokenCounter()
        window = ContextWindow(
            max_tokens=100, reserved_tokens=20, system_prompt_tokens=10
        )
    ...
    def test_context_window_sizing(self):
        ...
        small_window = ContextWindow(
            max_tokens=1000, reserved_tokens=200, system_prompt_tokens=100
        )
        medium_window = ContextWindow(
            max_tokens=2048, reserved_tokens=300, system_prompt_tokens=100
        )
        large_window = ContextWindow(
            max_tokens=8192, reserved_tokens=500, system_prompt_tokens=200
        )
...
class TestIntegrationConcepts:
    ...
    def test_realistic_conversation_scenario(self):
        ...
        for i in range(10):
            user_msg = ChatMessage(
                role="user", content=f"User message {i}: This is question number {i}"
            )
            assistant_msg = ChatMessage(
                role="assistant",
                content=f"Assistant response {i}: This is a detailed response to question {i} with helpful information",
            )
    ...
    def test_context_window_efficiency(self):
        ...
        messages = [
            ...
            ChatMessage(
                role="assistant",
                content="This is a medium length response with several words",
            ),
            ChatMessage(
                role="user",
                content="This is a very long user message that contains a lot of text and should consume many tokens when processed by the token counter system",
            ),
        ]
        ...
        large_window = ContextWindow(
            max_tokens=2000, reserved_tokens=300, system_prompt_tokens=100
        )
