...
class TestLLMFactory:
    ...
    def test_get_llm_unregistered_provider(self, clean_llm_registry):
        ...
        try:
            ...
            assert "LLM provider 'LLMProvider.OPENAI' not available" in str(
                exc_info.value
            )
...
class TestLLMFactoryEdgeCases:
    ...
    def test_concurrent_provider_creation(self, setup_test_providers):
        ...
        assert all(
            ...
            for instance in [openai_instance, anthropic_instance, groq_instance]
        )
        assert all(
    ...
    def test_factory_static_methods_work_without_instantiation(
        self, setup_test_providers
    ):
