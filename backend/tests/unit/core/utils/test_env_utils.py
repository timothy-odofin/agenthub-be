...
class TestEnvironmentVariableAccess:
    ...
    def test_get_required_non_existing_variable(self):
        ...
        with pytest.raises(
            ValueError,
            match="Required environment variable 'NON_EXISTENT_VAR' is not set",
        ):
...
class TestValueConversion:
    ...
    def test_convert_to_int_failure(self):
        ...
        with pytest.raises(
            ValueError, match="Cannot convert 'not_a_number' to integer"
        ):
...
class TestUtilityMethods:
    ...
    def test_list_variables_all(self):
        ...
        with patch.dict(
            os.environ, {"TEST_VAR1": "value1", "TEST_VAR2": "value2"}, clear=True
        ):
        ...
        with patch.object(manager, "_find_env_file", return_value=Path(".env")):
            ...
            def mock_import(name, *args, **kwargs):
                if name == "dotenv":
                    ...
                    mock_module.load_dotenv = MagicMock(
