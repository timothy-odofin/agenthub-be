...
class TestPropertyResolverComplexData:
    ...
    def test_resolve_nested_dict(self):
        ...
        data = {
            "server": {
                "host": "${TEST_HOST}",
                "port": "${TEST_PORT}",
                "config": {"debug": "${TEST_DEBUG}", "timeout": "${TEST_TIMEOUT:30}"},
            },
            "database": {"url": "jdbc:postgresql://${TEST_HOST}:${DB_PORT:5432}/mydb"},
        }
    ...
    def test_resolve_complex_mixed_structure(self):
        ...
        data = {
            "servers": [
                {"name": "primary", "host": "${TEST_HOST}", "port": "${TEST_PORT}"},
                {
                    "name": "secondary",
        ...
        expected = {
            "servers": [
                {"name": "primary", "host": "localhost", "port": 8080},
                {"name": "secondary", "host": "backup.example.com", "port": 9090},
            ],
            "config": {"debug": True, "features": ["auth", "logging", "monitoring"]},
        }
...
class TestPropertyResolverUtilityMethods:
    ...
    def test_extract_placeholder_variables(self):
        ...
        result = self.resolver.extract_placeholder_variables(
            "${VAR1:default1}_${VAR2:default2}"
        )
        assert result == ["VAR1", "VAR2"]
    ...
    def test_variable_has_default_in_data(self):
        ...
        assert (
            self.resolver._variable_has_default_in_data("VAR_WITH_DEFAULT", data)
            is True
        )
        assert (
            self.resolver._variable_has_default_in_data("VAR_WITHOUT_DEFAULT", data)
            is False
        )
        assert self.resolver._variable_has_default_in_data("VAR_MIXED", data) is True
        assert (
            self.resolver._variable_has_default_in_data("NONEXISTENT_VAR", data)
            is False
        )
