...
class TestSettingsEnhancements(unittest.TestCase):
    ...
    def test_to_dict_converts_all_profiles(
        self, mock_get_resources_dir, mock_load_file
    ):
        ...
        settings.app = DynamicConfig(
            {
                ...
                "security": DynamicConfig(
                    {"jwt_secret_key": "secret123", "jwt_algorithm": "HS256"}
                ),
            }
        )
        settings.db = DynamicConfig(
            {
                ...
                "redis": DynamicConfig({"host": "localhost", "port": 6379}),
            }
        )
    ...
    @patch("app.core.config.framework.settings.YamlLoader.load_file")
    @patch("app.core.config.framework.settings.Settings._get_resources_directory")
    def test_get_section_retrieves_nested_values(
        self, mock_get_resources_dir, mock_load_file
    ):
        ...
        settings.app = DynamicConfig(
            {
                "name": "test-app",
                "security": DynamicConfig(
                    {"jwt_secret_key": "secret123", "access_token_expire_minutes": 30}
                ),
            }
        )
        settings.db = DynamicConfig(
        ...
        missing_value = settings.get_section(
            "app.missing.path", default="default_value"
        )
