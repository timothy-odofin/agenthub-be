...
class TestAppConfig:
    ...
    def test_app_config_loads_ingestion_configuration_successfully(
        self, mock_validate, mock_convert, mock_settings, capsys
    ):
    ...
    def test_app_config_handles_no_data_sources(
        self, mock_settings_no_data_sources, capsys
    ):
        """Test that AppConfig handles missing data sources gracefully."""
        with patch(
            "app.core.config.application.app.settings", mock_settings_no_data_sources
        ):
            from app.core.config.application.app import AppConfig
    ...
    def test_app_config_handles_ingestion_config_validation_error(
        ...
        with patch("app.core.config.application.app.settings", mock_settings):
            ...
            captured = capsys.readouterr()
            assert (
                "Error loading ingestion config from Settings: Validation failed"
                in captured.out
            )

    @patch("app.core.config.application.app.dynamic_config_to_dict")
    def test_app_config_handles_conversion_error(
        self, mock_convert, mock_settings, capsys
    ):
        ...
        with patch("app.core.config.application.app.settings", mock_settings):
            ...
            captured = capsys.readouterr()
            assert (
                "Error loading ingestion config from Settings: Conversion failed"
                in captured.out
            )
