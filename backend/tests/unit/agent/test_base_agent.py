...
class ConcreteAgentImplementation(BaseAgent):
    ...
    def __init__(
        ...
        # If test_capabilities is provided (even if empty), use it exactly
        # Only use defaults if test_capabilities is None
        if test_capabilities is None:
            ...
            self._test_capabilities = test_capabilities
...
class TestBaseAgentInitialization:
    ...
    def test_initialization_with_required_params(self):
        ...
        agent = ConcreteAgentImplementation(
            agent_type=AgentType.PLANNING, framework=AgentFramework.LANGGRAPH
        )
...
class TestBaseAgentConcreteMethodsReturnTypes:
    ...
    def test_supports_capability_false(self, concrete_agent):
        ...
        assert (
            concrete_agent.supports_capability(AgentCapability.CODE_GENERATION) is False
        )
...
class TestBaseAgentWithDifferentConfigurations:
    ...
    def test_initialization_with_different_enum_values(self, agent_type, framework):
        ...
        # Act
        agent = ConcreteAgentImplementation(agent_type=agent_type, framework=framework)
    ...
    def test_custom_name_and_version(self):
        ...
        # Act
        agent = ConcreteAgentImplementation(
            test_name=custom_name, test_version=custom_version
        )
...
class TestBaseAgentBehaviorIntegration:
    ...
    async def test_multiple_execute_calls(self, concrete_agent, agent_context):
        ...
        assert all(
            r.status == AgentStatus.COMPLETED for r in [response1, response2, response3]
        )
...
class TestBaseAgentMockingAndDependencies:
    ...
    async def test_health_check_enum_value_access(self, concrete_agent):
        ...
        expected_capability_values = {
            cap.value for cap in concrete_agent.get_supported_capabilities()
        }
