...
class TestAgentContext:
    ...
    @pytest.mark.parametrize(
        "user_id", ["", "user123", "user@example.com", "user_with_underscores"]
    )
    ...
    def test_approval_callback_is_callable(self):
        ...
        # Act
        context = AgentContext(user_id="test", approval_callback=test_callback)
...
class TestAgentResponse:
    ...
    def test_minimal_initialization(self):
        ...
        # Act
        response = AgentResponse(content="Test response", status=AgentStatus.COMPLETED)
    ...
    def test_success_property_false_for_failures(self):
        ...
        for status in failure_statuses:
            ...
            ), f"Status {status} should be considered failure"
...
class TestToolResult:
    ...
    def test_minimal_initialization(self):
        ...
        # Act
        result = ToolResult(tool_name="test_tool", success=True, result="Tool output")
    ...
    def test_success_true_cases(self):
        ...
        for result_value, description in success_cases:
            ...
            tool_result = ToolResult(
                tool_name="test_tool", success=True, result=result_value
            )
    ...
    def test_various_tool_names(self, tool_name):
        ...
        # Act
        result = ToolResult(tool_name=tool_name, success=True, result="test output")
...
class TestAgentModelsIntegration:
    ...
    def test_agent_response_with_tool_results(self):
        ...
        tool_results = [
            ...
            ToolResult(
                tool_name="calculator", success=True, result=42, execution_time_ms=50.0
            ),
        ]
...
class TestAgentModelsEdgeCases:
    ...
    def test_agent_context_with_empty_tools_lists(self):
        ...
        # Act
        context = AgentContext(user_id="test", tools_allowed=[], tools_denied=[])
    ...
    def test_tool_result_with_complex_result_objects(self):
        ...
        # Act
        result = ToolResult(
            tool_name="complex_tool", success=True, result=complex_result
        )
    ...
    def test_timestamp_precision_and_ordering(self):
        ...
        for i in range(5):
            responses.append(
                AgentResponse(content=f"Response {i}", status=AgentStatus.COMPLETED)
            )

        # Assert - Timestamps should be in order or very close
        for i in range(1, len(responses)):
            time_diff = (
                responses[i].timestamp - responses[i - 1].timestamp
            ).total_seconds()
