...
from app.core.enums import AgentCapability, AgentFramework, AgentStatus, AgentType
...
class MockAgentForFactory(BaseAgent):
    ...
    def __init__(
        self, agent_type: AgentType, framework: AgentFramework, config=None, **kwargs
    ):
    ...
    async def execute(self, query: str, context: AgentContext) -> AgentResponse:
        return AgentResponse(
            content="Factory mock response", status=AgentStatus.COMPLETED
        )
...
class TestAgentFactory:
    ...
    @pytest.mark.asyncio
    async def test_create_agent_success_with_config(
        self, clean_registry, registered_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_agent_success_without_config(
        self, clean_registry, registered_agent
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_agent_success_with_kwargs_only(
        self, clean_registry, registered_agent
    ):
    ...
    async def test_create_agent_not_registered_raises_value_error(self, clean_registry):
        ...
        with pytest.raises(ValueError) as exc_info:
            await AgentFactory.create_agent(AgentType.REACT, AgentFramework.LANGCHAIN)

        error_message = str(exc_info.value)
        assert (
            "Agent type 'react' with framework 'langchain' not registered"
            in error_message
        )
        assert "Available:" in error_message

    @pytest.mark.asyncio
    async def test_create_agent_initialization_failure(
        self, clean_registry, failing_agent
    ):
        """Test agent creation when initialization fails."""
        # Arrange - Create a mock class that will fail during __init__
        with patch.object(
            failing_agent, "__init__", side_effect=ValueError("Init failed")
        ):
            # Act & Assert
            with pytest.raises(ValueError) as exc_info:
    ...
    async def test_create_agent_async_initialize_failure(self, clean_registry):
        ...
        with pytest.raises(RuntimeError) as exc_info:
            await AgentFactory.create_agent(
                AgentType.RESEARCH, AgentFramework.LANGCHAIN
            )
    ...
    @pytest.mark.asyncio
    async def test_create_agent_logs_error_on_failure(
        self, clean_registry, failing_agent
    ):
        """Test that agent creation failures are logged."""
        # Arrange
        with patch.object(
            failing_agent, "__init__", side_effect=ValueError("Test error")
        ):
            # Act & Assert
...
class TestAgentFactoryErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_create_agent_with_none_config(
        self, clean_registry, registered_agent
    ):
        """Test creating agent with explicitly None config."""
        # Act
        agent = await AgentFactory.create_agent(
            AgentType.REACT, AgentFramework.LANGCHAIN, config=None
        )
    ...
    async def test_create_agent_preserves_exception_type(self, clean_registry):
        ...
        with pytest.raises(TypeError) as exc_info:
            await AgentFactory.create_agent(AgentType.CUSTOM, AgentFramework.CUSTOM)
