...
class TestListSpacesTool(unittest.TestCase):
    ...
    def test_list_spaces_service_unavailable(self):
        ...
        with patch.object(
            type(provider), "confluence_service", new_callable=PropertyMock
        ) as mock_prop:
...
class TestGetPageTool(unittest.TestCase):
    ...
    def test_get_page_exception_handling(self):
        ...
        # Mock service to raise exception
        mock_service = Mock()
        mock_service.retrieve_confluence_page.side_effect = Exception(
            "Connection timeout"
        )
        provider._confluence_service = mock_service
...
class TestListPagesInSpaceTool(unittest.TestCase):
    ...
    def test_list_pages_exception_handling(self):
        ...
        # Mock service to raise exception
        mock_service = Mock()
        mock_service.list_confluence_pages_in_space.side_effect = Exception(
            "Invalid space"
        )
        provider._confluence_service = mock_service
...
class TestGetSpaceTool(unittest.TestCase):
    ...
    def test_get_space_success(self):
        ...
        mock_space_data = {
            ...
            "id": "12345",
            "type": "global",
            "description": {"plain": {"value": "Development space for the team"}},
            "homepage": {"id": "111", "title": "Dev Home"},
            "_links": {"webui": "/wiki/spaces/DEV"},
        }
        ...
        mock_confluence_client.get_space.assert_called_once_with(
            "DEV", expand="description.plain,homepage"
        )
