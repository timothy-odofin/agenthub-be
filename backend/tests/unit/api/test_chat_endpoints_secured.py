...
class TestSecuredChatEndpoints:
    ...
    async def test_send_message_with_auth(self, mock_user, mock_chat_service):
        ...
        # Arrange
        request = ChatRequest(message="Hello, AI!", session_id="session123")
    ...
    async def test_send_message_without_session_id(self, mock_user, mock_chat_service):
        ...
        # Arrange
        request = ChatRequest(message="Start new chat", session_id=None)
    ...
    async def test_create_session_with_auth(self, mock_user, mock_chat_service):
        ...
        mock_chat_service.create_session.assert_called_once_with(
            user_id="user123", title="My Chat Session"
        )
    ...
    async def test_get_session_history_with_auth(self, mock_user, mock_chat_service):
        ...
        # Arrange
        mock_chat_service.get_session_history = AsyncMock(
            return_value=[
                {
                    "role": "user",
                    "content": "Hello",
        ...
        response = await get_session_history(
            session_id="session123", current_user=mock_user, limit=50
        )
        ...
        mock_chat_service.get_session_history.assert_called_once_with(
            user_id="user123", session_id="session123", limit=50
        )

    @pytest.mark.asyncio
    async def test_get_session_history_with_custom_limit(
        self, mock_user, mock_chat_service
    ):
        ...
        # Act
        response = await get_session_history(
            session_id="session456", current_user=mock_user, limit=10
        )
        ...
        mock_chat_service.get_session_history.assert_called_once_with(
            user_id="user123", session_id="session456", limit=10
        )
    ...
    async def test_list_user_sessions_with_auth(self, mock_user, mock_chat_service):
        ...
        # Arrange
        mock_chat_service.list_user_sessions = MagicMock(
            return_value={
                "sessions": [
                    {
                        "session_id": "session1",
        ...
        # Act
        response = await list_user_sessions(current_user=mock_user, page=0, limit=10)
        ...
        mock_chat_service.list_user_sessions.assert_called_once_with(
            user_id="user123", page=0, limit=10
        )

    @pytest.mark.asyncio
    async def test_list_user_sessions_with_pagination(
        self, mock_user, mock_chat_service
    ):
        ...
        # Arrange
        mock_chat_service.list_user_sessions = MagicMock(
            return_value={"sessions": [], "total": 25, "has_more": True}
        )

        # Act
        response = await list_user_sessions(current_user=mock_user, page=2, limit=5)
        ...
        mock_chat_service.list_user_sessions.assert_called_once_with(
            user_id="user123", page=2, limit=5
        )
...
class TestChatSchemasUpdated:
    ...
    def test_chat_request_no_user_id(self):
        ...
        request = ChatRequest(message="Hello", session_id="session123")
