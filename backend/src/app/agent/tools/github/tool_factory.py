...
class GitHubToolFactory:
    ...
    def github_tools_config(self) -> Dict[str, Any]:
        ...
        if (
            not hasattr(settings, "tools")
            or not hasattr(settings.tools, "tools")
            or not hasattr(settings.tools.tools, "github")
        ):
    ...
    def create_repository_tools(
        ...
        logger.info(
            f"Created {len(filtered_tools)} tools for repository: {repository.full_name}"
        )
        return filtered_tools

    def _filter_tools_simplified(
        self, tools: List[StructuredTool], repository: GitHubRepository
    ) -> List[StructuredTool]:
        """
    ...
    def _create_enhanced_tool(
        self, original_tool: StructuredTool, repository: GitHubRepository
        ...
        try:
            ...
            enhanced_name = self._create_scoped_tool_name(
                tool_name, repository.full_name
            )

            # Enhance description with repository context
            enhanced_description = (
                f"{original_description} (Repository: {repository.full_name})"
            )
    ...
    def _create_scoped_tool_name(
        self, tool_name: str, repo_full_name: str, max_length: int = 64
    ) -> str:
        """
    ...
    def _get_repository_metadata(self, repository: GitHubRepository) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(
                f"Could not get repository metadata for {repository.full_name}: {e}"
            )
            return None
    ...
    def get_all_repository_tools(self) -> List[StructuredTool]:
        ...
        try:
            ...
            logger.info(
                f"Created total of {len(all_tools)} GitHub tools across {len(allowed_repositories)} repositories"
            )
