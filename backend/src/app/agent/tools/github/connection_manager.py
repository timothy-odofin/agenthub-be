...
from app.core.utils.file_utils import FileReadError, read_private_key_file
...
class GitHubConnectionManager:
    ...
    def github_config(self) -> Dict[str, Any]:
        ...
        if not hasattr(settings, "external") or not hasattr(
            settings.external, "github"
        ):
            ...
            "app": (
                {
                    ...
                    "private_key": (
                        github_config.app.private_key
                        if hasattr(github_config.app, "private_key")
                        else None
                    ),
                    "installation_id": github_config.app.installation_id,
                    "client_secret": github_config.app.client_secret,
                }
                if hasattr(github_config, "app")
                else {}
            ),
            "allowed_repositories": (
                github_config.allowed_repositories
                if hasattr(github_config, "allowed_repositories")
                else []
            ),
        }
    ...
    def _get_github_app_auth(self) -> Github:
        ...
        private_key_raw = app_config.get(
            "private_key"
        )  # Raw key content (for production)
        private_key_path = app_config.get(
            "private_key_path"
        ...
        if not app_id or (not private_key_raw and not private_key_path):
            ...
            if api_key:
                ...
                raise ValueError(
                    "No GitHub authentication method configured (App or PAT)"
                )
        ...
        if private_key_raw and private_key_raw != "None":
            ...
            raise ValueError(
                "No GitHub private key provided (neither raw content nor file path)"
            )
    ...
    def _get_installation(self) -> Installation:
        ...
        if not self._installation:
            ...
            if installation_id and installation_id != "None":
                ...
                self._installation = self._github_integration.get_app_installation(
                    int(installation_id)
                )
                ...
                logger.info(
                    f"Using first available installation: {self._installation.account.login} (ID: {self._installation.id})"
                )
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            if self.github_config.get("app"):
                ...
                installation = self._get_installation()
                logger.info(
                    f"GitHub App connected successfully to installation: {installation.account.login}"
                )
            else:
    ...
    def discover_accessible_repositories(self) -> List[GitHubRepository]:
        ...
        installation_id = (
            self._installation.id if self._installation else "no_installation"
        )
        ...
        if cache_key in _repo_discovery_cache:
            ...
            if age_seconds < _CACHE_TTL_SECONDS:
                ...
                logger.info(
                    f"Cache expired (age: {age_seconds:.1f}s > {_CACHE_TTL_SECONDS}s), refreshing..."
                )
        ...
        try:
            ...
            logger.info(
                f"Discovered and cached {len(accessible_repos)} accessible repositories"
            )
    ...
    def get_allowed_repositories(self) -> List[GitHubRepository]:
        ...
        if not allowed_repo_configs:
            logger.warning(
                "No allowed repositories configured, allowing all accessible repositories"
            )
            return accessible_repos
        ...
        logger.info(
            f"Filtered to {len(filtered_repos)} allowed repositories from {len(accessible_repos)} accessible"
        )
    ...
    def clear_repo_cache(installation_id: Optional[str] = None) -> None:
        ...
        if installation_id:
            ...
            if cache_key in _repo_discovery_cache:
                ...
                logger.info(
                    f"Cleared repository cache for installation: {installation_id}"
                )
        else:
    ...
    def get_connection_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "connected",
                "authentication": (
                    "github_app" if self.github_config.get("app") else "personal_token"
                ),
