...
class PrepareActionInput(BaseModel):
    ...
    tool_name: str = Field(
        ..., description="Name of the tool being executed (e.g., 'create_jira_issue')"
    )
    tool_args: Dict[str, Any] = Field(
        ..., description="Arguments for the tool (as a dictionary)"
    )
    ...
    session_id: Optional[str] = Field(
        None, description="Optional session ID for grouping actions"
    )
...
class ConfirmActionInput(BaseModel):
    ...
    action_id: str = Field(
        ..., description="ID of the action to confirm (from prepare_action response)"
    )
