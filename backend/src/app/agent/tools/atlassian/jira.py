...
class CreateIssueInput(BaseModel):
    ...
    issue_type: Optional[str] = Field(
        default="Task", description="Type of issue (Task, Bug, Story, etc.)"
    )
...
class AddCommentInput(BaseModel):
    ...
    issue_key: str = Field(
        description="The Jira issue key to add comment to (e.g., 'PROJ-123')"
    )
    comment_body: str = Field(description="The comment text to add to the issue")
    on_behalf_of: Optional[str] = Field(
        default=None,
        description="Username or email of the user on whose behalf this comment is being added",
    )
...
class GetAllUsersInput(BaseModel):
    ...
    start_at: Optional[int] = Field(
        default=0, description="Starting index for pagination"
    )
    max_results: Optional[int] = Field(
        default=50, description="Maximum number of results to return"
    )
...
class JiraTools:
    ...
    def _get_projects(self) -> str:
        ...
        if not self.jira_service:
            return json.dumps(
                {"status": "error", "error": "Jira service not available"}
            )
        ...
        result = {"status": "success", "total_projects": len(projects), "projects": []}
        ...
        for project in projects:
            project_info = {
                ...
                "lead": (
                    project.get("lead", {}).get("displayName", "Unknown")
                    if project.get("lead")
                    else "Unknown"
                ),
            }
    ...
    def _create_issue(
        ...
        if not project_key or not summary:
            return json.dumps(
                {"status": "error", "error": "project_key and summary are required"}
            )
    ...
    def _get_issue(self, issue_key: str) -> str:
        ...
        result = {"status": "success", "issue": issue}
    ...
    @handle_atlassian_errors()
    def _add_comment(
        self, issue_key: str, comment_body: str, on_behalf_of: Optional[str] = None
    ) -> str:
        ...
        if not is_adf and on_behalf_of:
            ...
            final_comment = (
                f"_On behalf of:_ *{on_behalf_of}*\n\n{comment_body_stripped}"
            )

        comment = self.jira_service.add_comment(
            issue_key=issue_key.strip(), comment_body=final_comment
        )
    ...
    def _get_project_users(
        ...
        users = self.jira_service.get_project_users(
            project_key=project_key.strip(), start_at=start_at, max_results=max_results
        )
    ...
    def create_comment_with_mentions(
        text_parts: List[Dict[str, str]], on_behalf_of: Optional[str] = None
    ) -> Dict:
        """
