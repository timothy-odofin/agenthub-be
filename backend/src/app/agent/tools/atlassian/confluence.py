...
class SearchPagesInput(BaseModel):
    ...
    query: str = Field(
        description="Search query text to find in page titles and content"
    )
    space_key: Optional[str] = Field(
        default=None, description="Optional space key to limit search (e.g., 'MYSPACE')"
    )
...
class ListPagesInSpaceInput(BaseModel):
    ...
    space_key: str = Field(description="The Confluence space key (e.g., 'MYSPACE')")
    max_results: Optional[int] = Field(
        default=50, description="Maximum number of pages to return"
    )
...
class ConfluenceTools:
    ...
    def _list_spaces(self, space_keys: Optional[List[str]] = None) -> str:
        ...
        try:
            ...
            if not service:
                return json.dumps(
                    {"status": "error", "message": "Confluence service not available"}
                )
            # Check if service is available before using it
            service = self.confluence_service
            if not service:
    ...
    def _get_page(self, page_id: str) -> str:
        ...
        try:
            ...
            if not page_id or not page_id.strip():
                return json.dumps({"status": "error", "message": "Page ID is required"})

            content, metadata = service.retrieve_confluence_page(page_id.strip())

            if not content:
                return json.dumps(
                    {"status": "error", "message": f"Could not retrieve page {page_id}"}
                )
    ...
    def _list_pages_in_space(self, space_key: str, max_results: int = 50) -> str:
        ...
        try:
            ...
            if not space_key or not space_key.strip():
                return json.dumps(
                    {"status": "error", "message": "Space key is required"}
                )
    ...
    def _search_pages(
        self, query: str, space_key: Optional[str] = None, max_results: int = 10
    ) -> str:
    ...
    def _get_space(self, space_key: str) -> str:
        ...
        try:
            ...
            # Use the underlying Confluence API client
            confluence_client = service._confluence
            space_data = confluence_client.get_space(
                space_key.strip(), expand="description.plain,homepage"
            )

            if not space_data:
                return json.dumps(
                    {"status": "error", "message": f"Could not find space {space_key}"}
                )
            ...
            result = {"status": "success", "space": space_info}
