...
class WebTools:
        ...
        if self._chunker is None:
            try:
                ...
                cache = CacheFactory.create_cache(
                    namespace="web_content", default_ttl=cache_ttl
                )
                ...
                logger.warning(
                    f"Failed to initialize cache, using chunker without cache: {e}"
                )
                self._chunker = WebContentChunker()
    ...
    async def _read_static_url(
        self, url: str, query: str, max_content_length: Optional[int] = None
    ) -> str:
        """
        ...
        try:
            ...
            documents = await self.chunker.fetch_and_chunk(
                url, use_cache=True, max_content_length=max_content_length
            )
    ...
    async def _read_dynamic_url(self, url: str, query: str = "") -> str:
        ...
        try:
            ...
            # Fetch and chunk content
            documents = await self.chunker.fetch_and_chunk(
                url, use_cache=True, max_content_length=max_length
            )
    ...
    def _search_chunks(self, documents: List, query: str) -> List:
        ...
        relevant = [doc for doc in documents if query_lower in doc.page_content.lower()]

        # Limit to top 5 most relevant chunks
        return relevant[:5]

    def _format_response(
        self, url: str, relevant_chunks: List, query: str, all_documents: List
    ) -> str:
        """
        ...
        for idx, chunk in enumerate(relevant_chunks, 1):
            section = {
                ...
                "relevance": (
                    "high"
                    if query and query.lower() in chunk.page_content.lower()
                    else "medium"
                ),
            }
