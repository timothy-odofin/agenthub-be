...
from datadog_api_client.v2.models import LogsListRequest, LogsListRequestPage, LogsSort
...
class DatadogAPIWrapper:
    ...
    def search_logs(
        ...
        try:
            with self._client() as api_client:
                ...
                logger.info(
                    f"Datadog logs search returned {len(out)} results for query: {query}"
                )
                return out
    ...
    def list_monitors(
        self, name_query: Optional[str] = None, tags: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
