...
from app.core.enums import AgentFramework, AgentType
...
class AgentRegistry:
    ...
    def register(cls, agent_type: AgentType, framework: AgentFramework):
        def decorator(agent_class: Type[BaseAgent]):
            ...
            logger.info(
                f"Registered agent: {agent_type.value} ({framework.value}) -> {agent_class.__name__}"
            )
            return agent_class
    ...
    @classmethod
    def get_agent_class(
        cls, agent_type: AgentType, framework: AgentFramework
    ) -> Type[BaseAgent]:
        key = (agent_type, framework)
        if key not in _agents:
            raise ValueError(
                f"No agent registered for type '{agent_type.value}' with framework '{framework.value}'"
            )
        return _agents[key]
    ...
    def get_agents_by_type(
        cls, agent_type: AgentType
    ) -> List[Tuple[AgentFramework, Type[BaseAgent]]]:
        return [
            (framework, agent_class)
            for (atype, framework), agent_class in _agents.items()
            if atype == agent_type
        ]

    @classmethod
    def get_agents_by_framework(
        cls, framework: AgentFramework
    ) -> List[Tuple[AgentType, Type[BaseAgent]]]:
        return [
            (atype, agent_class)
            for (atype, fw), agent_class in _agents.items()
            if fw == framework
        ]
