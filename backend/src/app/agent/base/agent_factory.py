    ...
    async def create_agent(
        ...
        **kwargs,
        ...
        if not AgentRegistry.is_registered(agent_type, framework):
            ...
            raise ValueError(
                f"Agent type '{agent_type.value}' with framework '{framework.value}' not registered. Available: {available}"
            )
        ...
        try:
            ...
            logger.info(
                f"Created and initialized agent: {agent.name} ({agent_type.value}/{framework.value})"
            )
            return agent

        except Exception as e:
            logger.error(
                f"Failed to create agent {agent_type.value}/{framework.value}: {e}",
                exc_info=True,
            )
            raise
        ...
        return [
            {
                ...
                "class_name": AgentRegistry.get_agent_class(
