...
class ConfluenceService(metaclass=SingletonMeta):
    ...
    def list_confluence_spaces(self, space_keys: list[str]) -> list[Any]:
        ...
        if "*" in space_keys:
            while True:
                spaces = self._confluence.get_all_spaces(
                    start=page_size, limit=limit, expand="description.plain,homepage"
                )
                logger.info(
                    f"Fetched spaces page starting at {page_size} from Confluence First Item, total Record {spaces['size']}"
                )
            ...
            for space_key in space_keys:
                space_data = self._confluence.get_space(
                    space_key, expand="description.plain,homepage"
                )
                if space_data:
                    logger.info(
                        f"Fetched space  with key {space_key} from Confluence First Item"
                    )
                    space_details.append(space_data)
                else:
                    logger.warning(
                        f"Could not find space {space_key} in Confluence First Item"
                    )
    ...
    def retrieve_confluence_page(self, page_id: str) -> Tuple[str, dict]:
        page = self._confluence.get_page_by_id(
            page_id, expand="body.storage,version,ancestors,space"
        )
    ...
    def list_confluence_pages_in_space(self, space_key: str) -> list[Any]:
        ...
        page_details = []
        while True:
            result = self._confluence.get_all_pages_from_space_raw(
                space_key,
                start=page_size,
                limit=limit,
            ...
            if result:
                pages = (
                    result.get(RESULT_KEY, []) if isinstance(result, dict) else result
                )
    ...
    def __init_client(self):
        ...
        if self._confluence is None:
            ...
            logger.info(
                f"Initializing Confluence client... with config: {atlassian_config.keys()}"
            )
