    ...
    async def generate_title(self, messages: List[ChatMessage]) -> str:
        ...
        try:
            ...
            provider = (
                ...
                else settings.llm.default.provider
            )
...
class ExtractiveTitleGenerationStrategy(TitleGenerationStrategy):
    ...
    def _extract_title_from_content(self, content: str) -> str:
        ...
        if (
            len(words) > self.config.max_title_words
            or len(content) > self.config.max_title_length
        ):
            title += "..."
...
class SessionTitleService:
    ...
    def should_generate_title(
        self, message_count: int, current_title: Optional[str] = None
    ) -> bool:
        """
        ...
        is_default_title = (
            not current_title
            or current_title_lower in default_titles
            or any(current_title_lower.startswith(dt) for dt in default_titles)
        )
    ...
    async def auto_generate_and_update_title(
        self, session_id: str, user_id: str, session_repository
    ) -> Optional[str]:
        """
        ...
        try:
            ...
            messages = await self._get_session_messages(
                session_id, user_id, session_repository
            )
            ...
            title = await self.generate_title_from_messages(
                messages, fallback_strategy=ExtractiveTitleGenerationStrategy()
            )

            # Update session with new title
            await self._update_session_title(
                session_id, user_id, title, session_repository
            )
    ...
    async def _update_session_title(
        self, session_id: str, user_id: str, title: str, session_repository
    ):
