...
class ConversationalAuthService:
    ...
    def __init__(self):
        ...
        self.extraction_config = (
            settings.workflows.signup.conversational_auth.extraction.universal
        )
        self.intent_config = (
            settings.workflows.signup.conversational_auth.intent_classification
        )
        self.clarifications = (
            settings.workflows.signup.conversational_auth.clarifications
        )
        self.validation_errors = (
            settings.workflows.signup.conversational_auth.validation_errors
        )
        ...
        self._step_handlers: Dict[
            SignupStep,
            Callable[
                [ConversationalSignupRequest, str],
                Awaitable[ConversationalSignupResponse],
            ],
        ] = {
    ...
    async def _extract_field_from_message(self, message: str, field_type: str) -> str:
        ...
        try:
            ...
            logger.info(
                f"LLM Extraction - Field: {field_type}, Input: '{message}', Extracted: '{extracted}'"
            )
            return extracted
    ...
    async def _classify_intent(self, message: str, field_type: str) -> str:
        """
        ...
        try:
            ...
            if intent not in valid_intents:
                logger.warning(
                    f"Invalid intent returned: {intent}, defaulting to PROVIDES_DATA"
                )
                return "PROVIDES_DATA"

            logger.info(
                f"Intent Classification - Field: {field_type}, Input: '{message}', Intent: {intent}"
            )
            return intent
    ...
    async def _classify_start_intent(self, message: str) -> str:
        """
        ...
        try:
            ...
            if intent not in valid_intents:
                logger.warning(
                    f"Invalid START intent returned: {intent}, defaulting to ASKS_INFO"
                )
                return "ASKS_INFO"

            logger.info(
                f"START Intent Classification - Input: '{message}', Intent: {intent}"
            )
            return intent
    ...
    async def _generate_intelligent_clarification(
        self, message: str, field_type: str
    ) -> str:
        """
        ...
        try:
            ...
            clarification_config = (
                settings.workflows.signup.conversational_auth.field_clarification
            )
            ...
            logger.info(
                f"Intelligent Clarification - Field: {field_type}, Question: '{message}', Response length: {len(clarification)}"
            )
    ...
    async def _generate_start_clarification(self, message: str) -> str:
        """
        ...
        try:
            ...
            clarification_config = (
                settings.workflows.signup.conversational_auth.start_clarification
            )
            ...
            logger.info(
                f"START Clarification - Question: '{message}', Response length: {len(clarification)}"
            )
            return clarification
        ...
        if current_step == SignupStep.START:
            ...
            if not request.message or request.message.strip() == "":
                ...
                await signup_session_repository.create_session(
                    session_id, {"current_step": SignupStep.EMAIL.value}
                )
            ...
            if intent == "ASKS_INFO":
                ...
                clarification = await self._generate_start_clarification(
                    request.message
                )
    ...
    async def _process_email(
        ...
        if intent == "CONFUSED":
            return ConversationalSignupResponse(
                success=False,
                message=self.validation_errors.extraction_failed.format(
        ...
        extracted_email = await self._extract_field_from_message(
            request.message, field_type="email address"
        )

        logger.info(
            f"Email validation - Extracted: '{extracted_email}', Pattern match: {bool(self.EMAIL_PATTERN.match(extracted_email))}"
        )
        ...
        await signup_session_repository.update_field(
            session_id, "email", extracted_email
        )
        await signup_session_repository.update_field(
            session_id, "current_step", SignupStep.USERNAME.value
        )
    ...
    async def _process_username(
        ...
        extracted_username = await self._extract_field_from_message(
            request.message, field_type="username"
        )
        ...
        await signup_session_repository.update_field(
            session_id, "username", extracted_username
        )
        await signup_session_repository.update_field(
            session_id, "current_step", SignupStep.PASSWORD.value
        )
    ...
    async def _process_password(
        ...
        extracted_password = await self._extract_field_from_message(
            request.message, field_type="password"
        )
        ...
        if not self.PASSWORD_PATTERN.match(extracted_password):
            ...
            error_msg = self.validation_errors.password_weak.format(
        ...
        await signup_session_repository.update_field(
            session_id, "password_hash", hashed_password
        )
        await signup_session_repository.update_field(
            session_id, "current_step", SignupStep.FIRSTNAME.value
        )
    ...
    async def _process_firstname(
        ...
        extracted_firstname = await self._extract_field_from_message(
            request.message, field_type="first name"
        )

        # Step 3: Validate name format
        if (
            not self.NAME_PATTERN.match(extracted_firstname)
            or len(extracted_firstname) < 1
        ):
            return ConversationalSignupResponse(
                success=False,
        ...
        await signup_session_repository.update_field(
            session_id, "firstname", extracted_firstname
        )
        await signup_session_repository.update_field(
            session_id, "current_step", SignupStep.LASTNAME.value
        )
    ...
    async def _process_lastname(
        ...
        extracted_lastname = await self._extract_field_from_message(
            request.message, field_type="last name"
        )

        # Step 3: Validate name format
        if (
            not self.NAME_PATTERN.match(extracted_lastname)
            or len(extracted_lastname) < 1
        ):
            return ConversationalSignupResponse(
                success=False,
        ...
        try:
            ...
            completion_message = self.prompts.complete.format(
                firstname=session_data.get("firstname")
            )
