...
from app.agent import AgentContext, AgentFactory
from app.core.constants import AgentFramework, AgentType
...
class ChatService(metaclass=SingletonMeta):
    ...
    async def set_agent_framework(self, framework: AgentFramework) -> None:
        if self._agent is not None:
            logger.warning(
                f"Switching agent framework from {self._agent_framework} to {framework}"
            )
    ...
    async def chat(
        ...
        try:
            ...
            logger.info(
                f"Processing chat message for user {user_id}, session {session_id} with provider={provider}, model={model}"
            )
            ...
            if metadata and metadata.get("is_capability_selection"):
                ...
                logger.info(
                    f"Enhanced message with capability context: {metadata.get('capability_id')}"
                )
            ...
            if model and hasattr(llm, "client") and hasattr(llm.client, "model_name"):
                logger.info(
                    f"Overriding LLM model from {llm.client.model_name} to {model}"
                )
            ...
            if provider or model:
                ...
                if agent is not None:
                    ...
                    logger.info(
                        f"Created and cached new agent for provider={provider}, model={model}"
                    )
            else:
            ...
            legacy_response = {
                ...
                "service": {
                    ...
                    "version": "2.0.0",
                    "protocol": protocol,
                    "service_processing_time_ms": round(
    ...
    def create_session(self, user_id: str, title: Optional[str] = None) -> str:
        try:
            ...
            session_id = session_repo.create_session(
                user_id=user_id, session_data={"title": session_title}
            )
    ...
    async def get_session_history(
        ...
        try:
            ...
            logger.error(
                f"Failed to get session history {session_id} for user {user_id}: {e}"
            )
            return []
    ...
    def list_user_sessions(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(formatted_sessions)} sessions for user {user_id}"
            )
            return {
    ...
    def _enhance_capability_message(
        ...
        try:
            ...
            if capability.get("tags"):
                enhanced_parts.append(
                    f"**Related Topics:** {', '.join(capability['tags'])}"
                )
                enhanced_parts.append("")
    ...
    async def _maybe_generate_title(self, user_id: str, session_id: str):
        ...
        try:
            ...
            should_generate = self.title_service.should_generate_title(
                user_message_count, session.title
            )
    ...
    async def update_session_title(
        ...
        try:
            ...
            session_repo.update_session(
                user_id=user_id, session_id=session_id, data={"title": title}
            )
    ...
    def delete_session(self, user_id: str, session_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Session {session_id} not found or unauthorized for user {user_id}"
                )
