...
class AgentThinking(LLMOutputBase):
    ...
    selected_action: Literal["respond", "use_tool", "ask_clarification", "escalate"] = (
        Field(..., description="Action the agent decided to take")
    )
    confidence_level: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in the decision"
    )
    requires_approval: bool = Field(
        default=False, description="Whether action needs approval"
    )
    tool_selection: Optional[str] = Field(None, description="Selected tool name if any")
    tool_parameters: Dict[str, Any] = Field(
        default_factory=dict, description="Parameters for tool"
    )
...
class ChatAgentResponse(LLMOutputBase):
    ...
    response_type: Literal["direct_answer", "tool_result", "clarification", "error"] = (
        Field(..., description="Type of response provided")
    )
    session_context_used: bool = Field(
        default=False, description="Whether session history was used"
    )
    ...
    needs_human_intervention: bool = Field(
        default=False, description="Whether human help is needed"
    )
    context_relevance: float = Field(
        default=0.0, ge=0.0, le=1.0, description="How relevant is the context"
    )
        ...
        json_schema_extra = {
            "example": {
                ...
                "follow_up_suggestions": [
                    "Would you like to check specific table status?"
                ],
...
class IngestionAnalysis(LLMOutputBase):
    ...
    quality_score: float = Field(
        ..., ge=0.0, le=1.0, description="Content quality assessment"
    )
    key_topics: List[str] = Field(
        default_factory=list, description="Main topics identified"
    )
    document_summary: str = Field(..., description="Brief summary of the document")
    suggested_tags: List[str] = Field(
        default_factory=list, description="Suggested metadata tags"
    )
...
class DataSourceProcessing(LLMOutputBase):
    ...
    preprocessing_steps: List[str] = Field(
        default_factory=list, description="Steps taken during preprocessing"
    )
...
class SystemDiagnostics(LLMOutputBase):
    ...
    component_status: Dict[str, str] = Field(
        default_factory=dict, description="Status of system components"
    )
    ...
    alert_level: Literal["green", "yellow", "red"] = Field(
        default="green", description="System alert level"
    )
...
class SessionAnalysis(LLMOutputBase):
    ...
    key_insights: List[str] = Field(
        default_factory=list, description="Important insights from conversation"
    )
    interaction_quality: float = Field(
        default=0.0, ge=0.0, le=1.0, description="Quality of interactions"
    )
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "key_insights": [
                    "User is implementing new API client",
                    "Needs rate limit best practices",
                ],
                "interaction_quality": 0.92,
                "suggested_actions": [
                    "Provide API client examples",
                    "Send rate limit monitoring guide",
                ],
...
class StructuredLLMResponse(BaseModel):
    ...
    structured_output: Optional[LLMOutputBase] = Field(
    ...
    processing_time_ms: Optional[float] = Field(
        None, description="Response generation time"
    )
