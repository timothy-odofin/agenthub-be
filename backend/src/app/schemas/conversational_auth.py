...
class ConversationalSignupRequest(BaseModel):
    ...
    message: str = Field(
        ...,
        description="User's message/input for current step",
        examples=["john@example.com"],
    )
    session_id: Optional[str] = Field(
        None,
        description="Session ID from server response (null only for START step)",
        examples=["abc-123"],
    )
    current_step: Optional[SignupStep] = Field(
        None,
        description="Current step (MUST match next_step from previous response, null only for START)",
        examples=["email"],
    )
...
class ConversationalSignupResponse(BaseModel):
    ...
    user_id: Optional[str] = Field(
        None, description="Created user ID if signup complete"
    )
    access_token: Optional[str] = Field(
        None, description="JWT access token if signup complete"
    )
    refresh_token: Optional[str] = Field(
        None, description="JWT refresh token if signup complete"
    )
    token_type: str = Field(default="bearer", description="Token type")

    # Progress tracking
    progress_percentage: int = Field(
        ..., description="Signup completion percentage (0-100)"
    )
...
class ConversationalLoginRequest(BaseModel):
    ...
    session_id: Optional[str] = Field(
        None, description="Session ID to track conversation state"
    )
...
class ConversationalLoginResponse(BaseModel):
    ...
    access_token: Optional[str] = Field(
        None, description="JWT access token if login successful"
    )
    refresh_token: Optional[str] = Field(
        None, description="JWT refresh token if login successful"
    )
    token_type: str = Field(default="bearer", description="Token type")
    user: Optional[dict] = Field(
        None, description="User information if login successful"
    )
