...
from app.core.resilience import get_all_circuit_breaker_stats, get_circuit_breaker_stats
from app.core.utils.logger import get_logger

# Import connection managers to ensure circuit breakers are initialized
try:
    from app.infrastructure.connections.external.confluence_connection_manager import (
        ConfluenceConnectionManager,
    )
    from app.infrastructure.connections.external.jira_connection_manager import (
...
async def get_circuit_breaker(circuit_name: str):
    ...
    if stats is None:
        raise NotFoundError(message=f"Circuit breaker '{circuit_name}' not found")
