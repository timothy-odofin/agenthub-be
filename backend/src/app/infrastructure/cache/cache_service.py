...
class CacheService:
    ...
    def __init__(
        ...
        self._provider = CacheFactory.create_cache(
            cache_type=cache_type, namespace=namespace, default_ttl=default_ttl
        )
        logger.debug(
            f"CacheService initialized with namespace '{namespace}', TTL={default_ttl}s"
        )
    ...
    async def get_by_index(
        self, index_name: str, index_value: str, deserialize: bool = True
    ) -> List[Any]:
        """
