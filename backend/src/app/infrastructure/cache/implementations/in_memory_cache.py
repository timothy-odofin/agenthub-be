...
class InMemoryCacheProvider(BaseCacheProvider):
    ...
    async def set(
        ...
        logger.debug(
            f"Set key '{key}' in namespace '{self.namespace}' with TTL {ttl_seconds}s"
        )
        return True
    ...
    async def update(
        ...
        if existing is None:
            logger.warning(
                f"Cannot update - key '{key}' not found in namespace '{self.namespace}'"
            )
            return False
    ...
    async def set_ttl(self, key: str, ttl: int) -> bool:
        ...
        logger.debug(
            f"Updated TTL for key '{key}' to {ttl}s in namespace '{self.namespace}'"
        )
        return True
    ...
    async def clear_namespace(self) -> int:
        ...
        async with self._lock:
            ...
            index_keys_to_delete = [
                k for k in self._indexes.keys() if k.startswith(prefix)
            ]
    ...
    @handle_cache_errors(
        operation="cleanup_expired", default_return=0, suppress_errors=True
    )
    async def cleanup_expired(self) -> int:
        ...
        if count > 0:
            logger.debug(
                f"Cleaned up {count} expired entries from namespace '{self.namespace}'"
            )
