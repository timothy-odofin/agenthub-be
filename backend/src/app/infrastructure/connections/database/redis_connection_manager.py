...
class RedisConnectionManager(AsyncBaseConnectionManager):
    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"Redis connection requires '{field}' in configuration"
                )
        ...
        if not isinstance(port, int) or port <= 0 or port > 65535:
            raise ValueError(
                f"Redis port must be a valid port number (1-65535), got: {port}"
            )
        ...
        if not isinstance(pool_size, int) or pool_size <= 0:
            raise ValueError(
                f"Redis connection_pool_size must be a positive integer, got: {pool_size}"
            )
    ...
    async def connect(self) -> Redis:
        ...
        try:
            ...
            logger.info(
                f"Redis connection established to {config_dict['host']}:{config_dict['port']}"
            )
            return self._redis_client
