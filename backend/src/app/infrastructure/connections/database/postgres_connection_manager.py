...
class PostgresConnectionManager(AsyncBaseConnectionManager):
    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"PostgreSQL connection requires '{field}' in configuration"
                )
        ...
        if not isinstance(port, int) or port <= 0 or port > 65535:
            raise ValueError(
                f"PostgreSQL port must be a valid port number (1-65535), got: {port}"
            )
        ...
        if not isinstance(pool_size, int) or pool_size <= 0:
            raise ValueError(
                f"PostgreSQL pool_size must be a positive integer, got: {pool_size}"
            )
    ...
    async def connect(self) -> Pool:
        ...
        try:
            ...
            logger.info(
                f"PostgreSQL connection pool established to {config_dict['host']}:{config_dict['port']}"
            )
            return self._connection_pool
