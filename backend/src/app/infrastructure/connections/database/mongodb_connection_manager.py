...
class MongoDBConnectionManager(BaseConnectionManager):
    ...
    def validate_config(self) -> None:
        ...
        if connection_string:
            ...
            if not connection_string.startswith(
                ...
                raise ValueError(
                    "MongoDB connection_string must start with 'mongodb://' or 'mongodb+srv://'"
                )
        else:
            ...
            for field in required_fields:
                if not config_dict.get(field):
                    raise ValueError(
                        f"MongoDB connection requires '{field}' in configuration when not using connection_string"
                    )
            ...
            if not isinstance(port, int) or port <= 0:
                raise ValueError(
                    f"MongoDB port must be a positive integer, got: {port}"
                )
    ...
    async def count_documents(
        ...
        try:
            ...
            logger.error(
                f"Failed to count documents in collection {collection_name}: {e}"
            )
            return 0
