...
from app.infrastructure.connections.base import (
...
class ConfluenceConnectionManager(BaseConnectionManager):
    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"Confluence connection requires '{field}' in atlassian configuration"
                )
        ...
        if not base_url.startswith(("http://", "https://")):
            raise ValueError(
                f"Confluence base_url must start with http:// or https://, got: {base_url}"
            )
        ...
        if not isinstance(timeout, int) or timeout <= 0:
            raise ValueError(
                f"Confluence timeout must be a positive integer, got: {timeout}"
            )
        ...
        if not isinstance(page_limit, int) or page_limit <= 0:
            raise ValueError(
                f"Confluence page_limit must be a positive integer, got: {page_limit}"
            )
        ...
        if not isinstance(space_keys, list):
            raise ValueError(
                "Confluence space_keys must be a string or list of strings"
            )
    ...
    def connect(self) -> Confluence:
        ...
        try:
            ...
            logger.info(
                f"Confluence connection established to {config_dict['confluence_base_url']}"
            )
            return self._confluence_client
    ...
    def get_pages_in_space(
        self, space_key: str, limit: Optional[int] = None
    ) -> List[Dict]:
        """
        ...
        return self._confluence_client.get_all_pages_from_space(
            space=space_key, start=0, limit=limit, expand="body.storage,metadata.labels"
        )
    ...
    def get_page_content(self, page_id: str) -> Optional[Dict]:
        ...
        try:
            return self._confluence_client.get_page_by_id(
                page_id=page_id, expand="body.storage,metadata.labels,version"
            )
        except Exception as e:
    ...
    def search_content(self, query: str, limit: Optional[int] = None) -> List[Dict]:
        ...
        try:
            return self._confluence_client.cql(
                cql=query, start=0, limit=limit, expand="content.body.storage"
            ).get("results", [])
        except Exception as e:
