...
class JiraConnectionManager(BaseConnectionManager):
    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"Jira connection requires '{field}' in atlassian configuration"
                )
        ...
        if not base_url.startswith(("http://", "https://")):
            raise ValueError(
                f"Jira base_url must start with http:// or https://, got: {base_url}"
            )
    ...
    def connect(self) -> Jira:
        ...
        try:
            ...
            logger.info(
                f"Connected to Jira server: {server_info.get('serverTitle', 'Unknown')} "
            ...
            logger.info(
                f"Jira connection established to {config_dict['jira_base_url']}"
            )
            return self._jira_client
    ...
    def search_issues(
        ...
        try:
            ...
            payload = {"jql": jql, "maxResults": limit}
    ...
    def get_all_users(self, start_at: int = 0, max_results: int = 50) -> List[Dict]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(users) if users else 0} users (start: {start_at}, limit: {max_results})"
            )
            return users or []
        except Exception as e:
    ...
    def get_project_users(
        ...
        try:
            ...
            users = self._jira_client.get(
                "/rest/api/3/user/assignable/search", params=params
            )
            logger.info(
                f"Retrieved {len(users) if users else 0} users for project {project_key}"
            )
            return users or []
        except Exception as e:
