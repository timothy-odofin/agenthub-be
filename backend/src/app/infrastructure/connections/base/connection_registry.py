...
class ConnectionRegistry:
    ...
    def register(cls, connection_type: ConnectionType):
        ...
        def decorator(connection_manager_class):
            ...
            logger.info(
                f"Registered connection manager: {connection_type} -> {connection_manager_class.__name__}"
            )
    ...
    @classmethod
    def get_connection_manager_class(
        cls, connection_type: ConnectionType
        ...
        if connection_type not in cls._registry:
            ...
            raise KeyError(
                f"Connection type '{connection_type}' not registered. Available: {available}"
            )
