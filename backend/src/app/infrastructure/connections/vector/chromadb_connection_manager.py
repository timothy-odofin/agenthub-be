    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"ChromaDB connection requires '{field}' in configuration"
                )
        ...
        if persist_directory:
            ...
            if not persist_path.is_dir():
                raise ValueError(
                    f"ChromaDB persist_directory must be a valid directory path: {persist_directory}"
                )
        ...
        try:
            ...
            if collection_name not in collection_names:
                ...
                self._chroma_client.create_collection(
                    name=collection_name, metadata={"created_at": str(datetime.now())}
                )
