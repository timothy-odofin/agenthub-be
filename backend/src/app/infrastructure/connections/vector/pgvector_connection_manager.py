...
class PgVectorConnectionManager(AsyncBaseConnectionManager):
    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"PgVector connection requires '{field}' in configuration"
                )
        ...
        if not connection_string.startswith("postgresql://"):
            raise ValueError(
                f"PgVector connection_string must start with postgresql://, got: {connection_string}"
            )
        ...
        if not isinstance(embedding_dimension, int) or embedding_dimension <= 0:
            raise ValueError(
                f"PgVector embedding_dimension must be a positive integer, got: {embedding_dimension}"
            )
        ...
        if distance_strategy not in valid_strategies:
            raise ValueError(
                f"PgVector distance_strategy must be one of {valid_strategies}, got: {distance_strategy}"
            )
    ...
    async def connect(self) -> asyncpg.Connection:
        ...
        try:
            ...
            if not result:
                ...
                try:
                    await self._pg_connection.execute(
            ...
            logger.info(
                f"PgVector connection established to {config_dict['connection_string']}"
            )
            return self._pg_connection
    ...
    async def _create_collection_table(
        self, collection_name: str, embedding_dimension: int
    ) -> None:
        ...
        try:
            ...
            if distance_threshold is not None:
                query = f"""
                    ...
                    LIMIT $2
                """
                results = await self._pg_connection.fetch(
                    query, embedding, limit, distance_threshold
                )
            else:
        ...
        if self._pg_connection:
            try:
                ...
                base_info.update(
                    {
                        "connection_string": (
                            self.config.get("connection_string", "").split("@")[-1]
                            if "@" in self.config.get("connection_string", "")
                            else "N/A"
                        ),  # Hide credentials
