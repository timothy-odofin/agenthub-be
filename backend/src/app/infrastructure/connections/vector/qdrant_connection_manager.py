    ...
    def validate_config(self) -> None:
        ...
        for field in required_fields:
            if not config_dict.get(field):
                raise ValueError(
                    f"Qdrant connection requires '{field}' in configuration"
                )
        ...
        if not url.startswith(("http://", "https://")):
            raise ValueError(
                f"Qdrant URL must start with http:// or https://, got: {url}"
            )
        ...
        if not isinstance(embedding_dimension, int) or embedding_dimension <= 0:
            raise ValueError(
                f"Qdrant embedding_dimension must be a positive integer, got: {embedding_dimension}"
            )
        ...
        if distance not in valid_distances:
            raise ValueError(
                f"Qdrant distance must be one of {valid_distances}, got: {distance}"
            )
                ...
                self._qdrant_client.create_collection(
                    ...
                    vectors_config=VectorParams(
                        ...
                        distance=distance_mapping.get(
                            config_dict.get("distance", "Cosine"), Distance.COSINE
                        ),
                    ),
                )
