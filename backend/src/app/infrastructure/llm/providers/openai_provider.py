...
class OpenAILLM(BaseLLMProvider):
    ...
    def validate_config(self) -> None:
        ...
        if model and model not in self.get_available_models():
            logger.warning(
                f"Model '{model}' may not be available. Available models: {self.get_available_models()}"
            )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"OpenAI LLM provider initialized with model: {self.client.model_name}"
            )
    ...
    async def _generate_impl(self, prompt: str, **kwargs) -> LLMResponse:
        ...
        try:
            ...
            if model_override:
                logger.info(
                    f"Using model override: {model_override} (default: {self.client.model_name})"
                )
    ...
    async def stream_generate(self, prompt: str, **kwargs) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            if model_override:
                logger.info(
                    f"Using model override for streaming: {model_override} (default: {self.client.model_name})"
                )
                kwargs["model"] = model_override
