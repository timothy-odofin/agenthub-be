...
class HuggingFaceLLM(BaseLLMProvider):
    ...
    def validate_config(self) -> None:
        ...
        if not model:
            raise ValueError(
                "HuggingFace provider requires 'default_model' in configuration"
            )
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"HuggingFace LLM provider initialized with model: {self.config.get('model', self.default_model)}"
            )
