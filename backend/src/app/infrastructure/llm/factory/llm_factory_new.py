...
class LLMFactory:
    ...
    @staticmethod
    async def clear_llm_cache(provider_name: Optional[str] = None, model: Optional[str] = None) -> int:
        """
        ...
        try:
            if provider_name is None:
                ...
                logger.warning("Clearing all LLM cache requires Redis SCAN or key tracking - not implemented yet")
                return 0
            elif model is None:
                # Get all keys for this provider using secondary index
                keys = await llm_provider_cache.get_keys_by_index("provider", provider_name)
                count = 0
                for key in keys:
                    if await llm_provider_cache.delete(key, indexes={"provider": provider_name}):
                        count += 1
                logger.info(f"Cleared {count} LLM cache entries for provider '{provider_name}'")
                return count
            else:
                # Clear specific provider+model combination
                cache_key = f"{provider_name}:{model}"
                if await llm_provider_cache.delete(cache_key, indexes={"provider": provider_name, "model": model}):
                    logger.info(f"Cleared LLM cache for provider '{provider_name}', model '{model}'")
                    return 1
                return 0
    ...
    async def get_llm_cache_stats() -> Dict:
        ...
        hit_rate = (LLMFactory._cache_hits / total_requests * 100) if total_requests > 0 else 0
    ...
    async def get_llm_by_name(
        ...
        if use_cache:
            try:
                ...
                logger.debug(f"Cached LLM provider: {provider_name}, model: {validated_model}")
            except Exception as e:
