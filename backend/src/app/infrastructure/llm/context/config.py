...
def get_context_window_config() -> ContextWindowConfig:
    ...
    try:
        ...
        if hasattr(settings, "context_window"):
            ...
            if strategies:
                if hasattr(strategies, "sliding"):
                    sliding_config = {
                        "min_recent_messages": getattr(
                            strategies.sliding, "min_recent_messages", 5
                        )
                    }

                if hasattr(strategies, "summarization"):
                    summarization_config = {
                        "summarization_threshold": getattr(
                            strategies.summarization, "summarization_threshold", 20
                        ),
                        "keep_recent_messages": getattr(
                            strategies.summarization, "keep_recent_messages", 10
                        ),
                    }
            ...
            if performance_config:
                enable_token_caching = getattr(
                    performance_config, "enable_token_caching", True
                )
                ...
                max_processing_time = getattr(
                    performance_config, "max_processing_time", 2.0
                )
            ...
            if safety_config:
                min_preserved_messages = getattr(
                    safety_config, "min_preserved_messages", 2
                )
                max_single_message_tokens = getattr(
                    safety_config, "max_single_message_tokens", 2000
                )
                emergency_message_limit = getattr(
                    safety_config, "emergency_message_limit", 5
                )
        ...
        logger.warning(
            f"Could not load context window config from Settings: {e}, using defaults"
        )
        return ContextWindowConfig()
