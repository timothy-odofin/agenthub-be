...
from app.core.resilience import RetryConfig, RetryStrategy, async_retry, retry
...
class MongoSessionRepository(BaseSessionRepository):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    self._run_create_session_sync, session_id, user_id, session_data
                )
    ...
    def _run_create_session_sync(
        ...
        try:
            loop.run_until_complete(
                self._create_session_async(session_id, user_id, session_data)
            )
        finally:
            loop.close()
    ...
    async def ensure_session_exists(
        self, session_id: str, user_id: str, session_data: dict = None
    ) -> bool:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            existing_session = await asyncio.get_event_loop().run_in_executor(
    ...
    def get_session(self, user_id: str, session_id: str) -> ChatSession:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    self._run_get_session_sync, user_id, session_id
                )
    ...
    def get_session_messages(
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    self._run_get_session_messages_sync, user_id, session_id, limit
                )
                return future.result()
        except RuntimeError:
            # No running event loop - safe to run directly
            return asyncio.run(
                self._get_session_messages_async(user_id, session_id, limit)
            )

    def _run_get_session_messages_sync(
    ...
    @async_retry(MONGODB_RETRY_CONFIG)
    async def _get_session_messages_async(
        self, user_id: str, session_id: str, limit: int
    ) -> List[ChatMessage]:
        ...
        def find_messages():
            ...
            cursor = (
                self._messages_collection.find({"session_id": session_id})
                .sort("timestamp", 1)
                .limit(limit)
            )
    ...
    async def get_session_history(
        ...
        def get_session_and_messages():
            ...
            # Get messages
            cursor = self._messages_collection.find({"session_id": session_id}).sort(
                "timestamp", 1
            )
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            return await asyncio.get_event_loop().run_in_executor(
                executor, get_session_and_messages
            )
    ...
    def update_session(self, user_id: str, session_id: str, data: dict) -> bool:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    self._run_update_session_sync, user_id, session_id, data
                )
                return future.result()
        except RuntimeError:
            # No running event loop - safe to run directly
    ...
    def _run_update_session_sync(
        ...
        try:
            return loop.run_until_complete(
                self._update_session_async(user_id, session_id, data)
            )
        finally:
            loop.close()
    ...
    async def _update_session_async(
        ...
        def update_session():
            return self._sessions_collection.update_one(
                {"session_id": session_id, "user_id": user_id}, {"$set": update_doc}
            )

        with concurrent.futures.ThreadPoolExecutor() as executor:
            result = await asyncio.get_event_loop().run_in_executor(
                executor, update_session
            )

        return result.modified_count > 0

    def list_paginated_sessions(
        self, user_id: str, page: int = 0, limit: int = 10
    ) -> List[ChatSession]:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    self._run_list_sessions_sync, user_id, page, limit
                )
                return future.result()
        except RuntimeError:
            # No running event loop - safe to run directly
    ...
    def _run_list_sessions_sync(
        ...
        try:
            return loop.run_until_complete(
                self._list_sessions_async(user_id, page, limit)
            )
        finally:
            loop.close()
    ...
    async def _list_sessions_async(
        ...
        def list_sessions():
            cursor = (
                self._sessions_collection.find({"user_id": user_id})
                .sort("updated_at", -1)
                .skip(skip)
                .limit(limit)
            )
    ...
    def delete_session(self, user_id: str, session_id: str) -> bool:
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    self._run_delete_session_sync, user_id, session_id
                )
                return future.result()
        except RuntimeError:
            # No running event loop - safe to run directly
    ...
    def _run_delete_session_sync(self, user_id: str, session_id: str) -> bool:
        ...
        try:
            return loop.run_until_complete(
                self._delete_session_async(user_id, session_id)
            )
        finally:
            loop.close()
    ...
    async def _delete_session_async(self, user_id: str, session_id: str) -> bool:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            return await asyncio.get_event_loop().run_in_executor(
                executor, delete_session_and_messages
            )
