...
class SignupSessionRepository:
    ...
    async def create_session(
        self, session_id: str, initial_data: Optional[Dict[str, Any]] = None
    ) -> bool:
        """
        ...
        try:
            ...
            await self.cache.set(key=key, value=session_data, ttl=self.SESSION_TTL)
            ...
            logger.error(
                f"Failed to create signup session {session_id}: {e}", exc_info=True
            )
            return False
    ...
    async def update_field(self, session_id: str, field: str, value: Any) -> bool:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to update field in session {session_id}: {e}", exc_info=True
            )
            return False
    ...
    async def extend_ttl(self, session_id: str, seconds: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if result:
                ...
                logger.warning(
                    f"Failed to extend TTL - session may not exist: {session_id}"
                )
            ...
            logger.error(
                f"Failed to extend TTL for session {session_id}: {e}", exc_info=True
            )
            return False
    ...
    async def increment_attempt(self, session_id: str) -> int:
        ...
        try:
            ...
            logger.error(
                f"Failed to increment attempt for session {session_id}: {e}",
                exc_info=True,
            )
            return 0
