...
class PendingActionsStore:
    ...
    def __init__(self, ttl_minutes: int = 10):
        ...
        logger.info(
            f"PendingActionsStore initialized with TTL={ttl_minutes} minutes (using confirmation_cache)"
        )
    ...
    async def get_user_actions(
        ...
        """
        action_data_list = await confirmation_cache.get_by_index(
            self.USER_INDEX_PREFIX, user_id
        )
