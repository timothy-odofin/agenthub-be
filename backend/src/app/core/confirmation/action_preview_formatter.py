...
def format_email(params: Dict[str, Any]) -> str:
    ...
    if attachments:
        attachment_list = (
            ", ".join(attachments) if isinstance(attachments, list) else attachments
        )
        lines.extend(
            [
                "",
...
def get_default_formatter() -> ActionPreviewFormatter:
    ...
    formatter.register_formatter(
        "jira", "create_jira_issue", format_jira_issue_creation
    )
    ...
    formatter.register_formatter(
        "github", "create_github_issue", format_github_issue_creation
    )
