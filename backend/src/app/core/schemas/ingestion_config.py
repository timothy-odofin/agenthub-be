...
class DataSourceConfig(BaseModel):
    ...
    sources: List[str] = Field(
        ..., min_items=1, description="List of sources identifiers (paths, URLs, IDs)"
    )
    location: Optional[AnyUrl] = Field(
        None, description="Base location for the sources (e.g. Confluence URL)"
    )
        ...
        if isinstance(v, list):
            ...
            for item in v:
                if isinstance(item, dict):
                    ...
                    if key in result:
                        raise ValueError(
                            f"Duplicate data source type found: {source_type}"
                        )
                    result[key] = item
                else:
