...
class TokenManager:
    ...
    def __init__(self) -> None:
        ...
        if not hasattr(self, "_initialized"):
            ...
            try:
                self._secret_key = settings.get_value(
                    "security.jwt_secret_key", "your-secret-key-change-in-production"
                )
                self._algorithm = settings.get_value("security.jwt_algorithm", "HS256")
                self._access_token_expire_minutes = int(
                    settings.get_value("security.access_token_expire_minutes", 30)
                )
            except Exception as e:
                logger.warning(
                    f"Could not load JWT config from settings: {e}. Using defaults."
                )
            ...
            logger.info(
                f"TokenManager initialized (algorithm: {self._algorithm}, expiry: {self._access_token_expire_minutes}min)"
            )
    ...
    def create_refresh_token(
        ...
        """
        try:
            to_encode = {"sub": user_id, "user_id": user_id, "type": "refresh"}
    ...
    def verify_refresh_token(self, token: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error(
                f"Unexpected error verifying refresh token: {e}", exc_info=True
            )
            return None
