...
from ...utils import PropertyResolver, property_resolver
...
class Settings:
    ...
    def _load_profile_configurations(self):
        ...
        try:
            ...
            if not profile_data:
                logger.warning(
                    "No profile configuration files found. Using empty configuration."
                )
            ...
            logger.info(
                f"Successfully loaded {len(all_profiles_data)} profile configurations: {list(all_profiles_data.keys())}"
            )
    ...
    def _load_flat_profile(
        self, profile_name: str, file_path: str, all_profiles_data: Dict[str, Any]
    ):
        ...
        try:
            ...
            logger.error(
                f"Error loading flat profile '{profile_name}' from {file_path}: {e}"
            )

    def _load_nested_profile(
        self, namespace: str, configs: Dict[str, str], all_profiles_data: Dict[str, Any]
    ):
        ...
        for config_name, file_path in configs.items():
            try:
                ...
                if config_data and isinstance(config_data, dict):
                    ...
                    logger.debug(
                        f"Loaded nested profile '{namespace}.{config_name}' from {file_path}"
                    )
                else:
                    logger.warning(
                        f"Profile file {file_path} is empty or not a dictionary"
                    )
            except Exception as e:
                logger.error(
                    f"Error loading nested profile '{namespace}.{config_name}' from {file_path}: {e}"
                )
    ...
    def _get_resources_directory(self) -> Path:
        ...
        if not resources_dir.exists():
            ...
            if not resources_dir.exists():
                ...
                logger.warning(
                    f"Could not find resources directory. Checked: {project_root / 'resources'} and {Path.cwd() / 'resources'}"
                )
    ...
    def _discover_profile_files(self, resources_dir: Path) -> Dict[str, Any]:
        ...
        self._discover_flat_profiles(
            resources_dir, profile_data, load_all, target_profiles
        )

        # 2. Discover nested application-*.yaml files in subdirectories (new feature)
        self._discover_nested_profiles(
            resources_dir, profile_data, load_all, target_profiles
        )
        ...
        if not load_all:
            ...
            for key, value in profile_data.items():
                if isinstance(value, dict):
                    all_profile_names.update(
                        f"{key}.{sub_key}" for sub_key in value.keys()
                    )
                    ...
                    if filename.startswith("application-"):
                        ...
                        if profile_name:  # Make sure there's a profile name
                            ...
                            if load_all or profile_name in target_profiles:
                                ...
                                logger.debug(
                                    f"Discovered flat profile '{profile_name}' at {yaml_file}"
                                )
                            else:
                                logger.debug(
                                    f"Skipping flat profile '{profile_name}' (not in PROFILES list)"
                                )

        except Exception as e:
            logger.error(
                f"Error discovering flat profile files in {resources_dir}: {e}"
            )

    def _discover_nested_profiles(
        self,
        resources_dir: Path,
        profile_data: Dict[str, Any],
        load_all: bool,
            ...
            for item in resources_dir.iterdir():
                if item.is_dir():
                        ...
                        if yaml_file.is_file():
                            ...
                            if filename.startswith("application-"):
                                ...
                                if config_name:
                                    ...
                                    if (
                                        load_all
                                        or folder_name in target_profiles
                                        or full_profile_name in target_profiles
                                    ):
                                        folder_configs[config_name] = str(yaml_file)
                                        logger.debug(
                                            f"Discovered nested profile '{folder_name}.{config_name}' at {yaml_file}"
                                        )
                                    else:
                                        logger.debug(
                                            f"Skipping nested profile '{full_profile_name}' (not in PROFILES list)"
                                        )
                    ...
                    if folder_configs:
                        ...
                        logger.info(
                            f"Discovered nested profile namespace '{folder_name}' with {len(folder_configs)} configs"
                        )

        except Exception as e:
            logger.error(
                f"Error discovering nested profile files in {resources_dir}: {e}"
            )
    ...
    def _create_dynamic_attributes(self, data: Dict[str, Any]):
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict) and any(
                ...
                logger.debug(
                    f"Created nested dynamic attribute '{attr_name}' with configs: {list(value.keys())}"
                )
        ...
        logger.info(
            f"Created dynamic attributes for profiles: {self._configured_profiles}"
        )
    ...
    def reload_profile(self, profile_name: str):
        ...
        try:
            ...
            if profile_data:
                ...
                logger.info(
                    f"Reloaded and resolved profile '{profile_name}' from {file_path}"
                )
            else:
                logger.warning(f"Profile file {file_path} is empty after reload")
        except Exception as e:
            logger.error(
                f"Error reloading profile '{profile_name}' from {file_path}: {e}"
            )
    ...
    def validate_environment_variables(self) -> Dict[str, List[str]]:
        ...
        for profile_name, file_path in self._profile_files.items():
            try:
                ...
                if raw_data:
                    # Extract variables
                    profile_vars = self._property_resolver._extract_all_variables(
                        raw_data
                    )
                    ...
                    for var in profile_vars:
                        if self._property_resolver._variable_has_default_in_data(
                            var, raw_data
                        ):
                ...
                logger.warning(
                    f"Could not scan profile '{profile_name}' for variables: {e}"
                )
        ...
        missing_vars = [
            var
            for var in all_vars
            if not env.has(var) and var not in vars_with_defaults
        ]
        ...
        for profile_name, file_path in self._profile_files.items():
            try:
                ...
                if raw_data:
                    ...
                    if profile_vars:
                        placeholder_info["profiles_with_placeholders"][
                            profile_name
                        ] = profile_vars
                ...
                logger.warning(
                    f"Could not analyze placeholders in profile '{profile_name}': {e}"
                )

        placeholder_info["unique_variables"] = list(
            placeholder_info["unique_variables"]
        )
    ...
    def reload_profiles(self):
        ...
        self._load_profile_configurations()
        logger.info(
            f"Profile reload complete. Active profiles: {self.get_profile_names()}"
        )
