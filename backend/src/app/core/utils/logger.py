...
from app.core.utils.formatters import HumanReadableFormatter, JSONFormatter
    ...
    if include_console:
        ...
        if log_format == LogFormat.JSON:
            console_formatter = JSONFormatter(
                service_name=service_name, environment=environment, include_extra=True
            )
        else:
...
def log_async_start(
    logger: logging.Logger, operation: str, context: Optional[Dict[str, Any]] = None
) -> None:
    """
...
def log_request(
    ...
    log_with_context(
        logger, level, f"{method} {path} - {status_code} ({duration_ms:.2f}ms)", context
    )
