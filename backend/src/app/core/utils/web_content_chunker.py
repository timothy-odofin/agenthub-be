...
class WebContentChunker:
    ...
    async def fetch_and_chunk(
        self, url: str, use_cache: bool = True, max_content_length: Optional[int] = None
    ) -> List[Document]:
        """
        ...
        if use_cache and self.cache_provider:
            ...
            if cached_docs:
                logger.info(
                    f"Retrieved {len(cached_docs)} chunks from cache for: {url}"
                )
                return cached_docs
        ...
        document = Document(page_content=cleaned_text, metadata=base_metadata)
        ...
        logger.info(
            f"Successfully chunked content from {url} into {len(chunks)} pieces"
        )
    ...
    async def _fetch_content(self, url: str) -> str:
        ...
        try:
            ...
            error_msg = (
                str(e)
                if "Failed to fetch URL content" in str(e)
                else f"Failed to fetch URL content: {str(e)}"
            )
    ...
    async def _save_to_cache(self, url: str, documents: List[Document]) -> bool:
        ...
        try:
            ...
            success = await self.cache_provider.set(
                cache_key, serializable_docs, ttl=self.cache_ttl
            )

            if success:
                logger.debug(
                    f"Cached {len(documents)} chunks for {url} (TTL: {self.cache_ttl}s)"
                )
