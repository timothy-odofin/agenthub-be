...
class PropertyResolver:
    ...
    def resolve_list(self, data: List[Any]) -> List[Any]:
        ...
        return [
            (
                self.resolve_dict(item)
                if isinstance(item, dict)
                else (
                    self.resolve_list(item)
    ...
    def _resolve_string_placeholders(self, value: str) -> str:
        ...
        if resolved == "__PLACEHOLDER_NONE__" and self.PLACEHOLDER_PATTERN.fullmatch(
            value
        ):
            return None
    ...
    def validate_required_variables(
        self, data: Dict[str, Any], required_vars: Optional[List[str]] = None
    ) -> List[str]:
    ...
    def _variable_has_default_in_data(self, var_name: str, data: Any) -> bool:
        ...
        if isinstance(data, dict):
            return any(
                self._variable_has_default_in_data(var_name, value)
                for value in data.values()
            )
        elif isinstance(data, list):
            return any(
                self._variable_has_default_in_data(var_name, item) for item in data
            )
        elif isinstance(data, str):
            ...
            for match in matches:
                ...
                if (
                    match[0].strip() == var_name
                    and len(match) > 1
                    and match[1] is not None
                    and ":" in data
                ):  # Ensure there was actually a colon in the original string
                    return True
