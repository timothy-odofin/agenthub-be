...
Tests the in-memory cache implementation including all operations,
TTL handling, index operations, and error handling.
"""
...
@pytest.fixture
def cache():
    """Create an in-memory cache instance for testing."""
    return InMemoryCacheProvider(namespace="test", default_ttl=60)


@pytest.fixture
def cache_no_ttl():
    """Create an in-memory cache instance without TTL."""
    return InMemoryCacheProvider(namespace="test_no_ttl", default_ttl=None)
...
class TestInMemoryCacheTTL:
    ...
    @pytest.mark.asyncio
    async def test_expiration_removes_key(self, cache):
    ...
    @pytest.mark.asyncio
    async def test_no_ttl_key_never_expires(self, cache_no_ttl):
        ...
        # Should still exist
        assert await cache_no_ttl.exists("key1") is True
        value = await cache_no_ttl.get("key1")
        assert value == "value1"


class TestInMemoryCacheIndexOperations:
    ...
    @pytest.mark.asyncio
    async def test_expired_key_removed_from_indexes(self, cache):
        """Test that expired keys are removed from indexes."""
        indexes = {"user_id": "123"}
        await cache.set("session:abc", {"data": "value"}, ttl=1, indexes=indexes)
        ...
        # Should be removed from index
        keys = await cache.get_by_index("user_id", "123")
...
class TestInMemoryCacheIncrement:
    ...
    @pytest.mark.asyncio
    async def test_increment_with_ttl(self, cache):
        """Test incrementing with TTL."""
        result = await cache.increment("counter", ttl=120)
        assert result == 1
...
class TestInMemoryCacheClearNamespace:
    ...
    @pytest.mark.asyncio
    async def test_clear_empty_namespace(self, cache):
...
class TestInMemoryCacheCleanup:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_removes_expired_keys(self, cache):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_with_no_expired_keys(self, cache):
...
class TestInMemoryCacheErrorHandling:
    """Test error handling with decorator."""
    
    @pytest.mark.asyncio
    async def test_error_handling_with_suppress_errors(self, cache):
        """Test that errors are suppressed and default values returned."""
        # All methods have suppress_errors=True, so they should return default values
        # We can't easily simulate errors without mocking internals
        # This test verifies the decorator is applied
        
        # These should not raise exceptions even if something goes wrong
